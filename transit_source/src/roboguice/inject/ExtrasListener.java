...
//            InjectExtra, ExtraConverter, Nullable

public class ExtrasListener
    implements TypeListener
{
    protected static class ExtrasMembersInjector
        implements MembersInjector
    {
        ...
        public ExtrasMembersInjector(Field field1, Provider provider, InjectExtra injectextra)
        {
            field = field1;
    ...
    public ExtrasListener(Provider provider)
    {
        contextProvider = provider;
    }
    ...
    public void hear(TypeLiteral typeliteral, TypeEncounter typeencounter)
    {
        for (typeliteral = typeliteral.getRawType(); typeliteral != java/lang/Object; typeliteral = typeliteral.getSuperclass())
        {
            ...
            for (int i = 0; i < j; i++)
            {
                ...
                if (!field.isAnnotationPresent(roboguice/inject/InjectExtra))
                {
                    continue;
                }
                ...
                typeencounter.register(new ExtrasMembersInjector(field, contextProvider, (InjectExtra)field.getAnnotation(roboguice/inject/InjectExtra)));
            }
