...
//            InjectResource, Nullable

public class ResourceListener
    implements TypeListener
{
    protected static class ResourceMembersInjector
...
_L3:
        ...
        public ResourceMembersInjector(Field field1, Application application1, InjectResource injectresource)
        {
            field = field1;
    ...
    public ResourceListener(Application application1)
    {
        application = application1;
    }
    ...
    public void hear(TypeLiteral typeliteral, TypeEncounter typeencounter)
    {
        for (typeliteral = typeliteral.getRawType(); typeliteral != java/lang/Object; typeliteral = typeliteral.getSuperclass())
        {
            ...
            for (int i = 0; i < j; i++)
            {
                ...
                if (field.isAnnotationPresent(roboguice/inject/InjectResource) && !Modifier.isStatic(field.getModifiers()))
                {
                    typeencounter.register(new ResourceMembersInjector(field, application, (InjectResource)field.getAnnotation(roboguice/inject/InjectResource)));
                }
            }
    ...
    public transient void requestStaticInjection(Class aclass[])
    {
        ...
        for (int i = 0; i < k; i++)
        {
            for (Class class1 = aclass[i]; class1 != java/lang/Object; class1 = class1.getSuperclass())
            {
                ...
                for (int j = 0; j < l; j++)
                {
                    ...
                    if (Modifier.isStatic(field.getModifiers()) && field.isAnnotationPresent(roboguice/inject/InjectResource))
                    {
                        (new ResourceMembersInjector(field, application, (InjectResource)field.getAnnotation(roboguice/inject/InjectResource))).injectMembers(null);
                    }
                }
