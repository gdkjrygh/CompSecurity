...
//            InjectView, InjectFragment, Nullable

public class ViewListener
    implements TypeListener
{
    public static class ViewMembersInjector
        implements MembersInjector
    {
        ...
        protected static void injectViews(Object obj)
        {
            ...
            for (obj1 = ((ArrayList) (obj1)).iterator(); ((Iterator) (obj1)).hasNext(); ((ViewMembersInjector)((Iterator) (obj1)).next()).reallyInjectMembers(obj)) { }
            break MISSING_BLOCK_LABEL_54;
            obj;
            roboguice/inject/ViewListener$ViewMembersInjector;
...
_L6:
        ...
        public ViewMembersInjector(Field field1, Annotation annotation1, TypeEncounter typeencounter)
        {
            field = field1;
    ...
    {
        typeliteral = typeliteral.getRawType();
        do
        {
            if (typeliteral == java/lang/Object)
            {
                break;
            }
            ...
            while (i < j) 
            {
                ...
                if (field.isAnnotationPresent(roboguice/inject/InjectView))
                {
                    ...
                    if (android/content/Context.isAssignableFrom(field.getDeclaringClass()) && !android/app/Activity.isAssignableFrom(field.getDeclaringClass()))
                    {
                        throw new UnsupportedOperationException("You may only use @InjectView in Activity contexts");
                    }
                    typeencounter.register(new ViewMembersInjector(field, field.getAnnotation(roboguice/inject/InjectView), typeencounter));
                } else
                if (field.isAnnotationPresent(roboguice/inject/InjectFragment))
                {
                    if (Modifier.isStatic(field.getModifiers()))
                    {
                    ...
                    typeencounter.register(new ViewMembersInjector(field, field.getAnnotation(roboguice/inject/InjectFragment), typeencounter));
                }
                i++;
            }
            typeliteral = typeliteral.getSuperclass();
        } while (true);
    }
    ...
    static 
    {
        ...
        try
        {
            ...
            fragmentFindFragmentByIdMethod = fragmentManagerClass.getMethod("findFragmentById", new Class[] {
                Integer.TYPE
            });
            fragmentFindFragmentByTagMethod = fragmentManagerClass.getMethod("findFragmentByTag", new Class[] {
                java/lang/Object
            });
        }
