    ...
    public View onCreateView(String s, Context context, AttributeSet attributeset)
    {
        if ("fragment".equals(s))
        {
            ...
            if (Fragment.isSupportFragmentClass(mActivity, s1))
            {
                ...
                if (DEBUG)
                {
                    Log.v("FragmentManager", (new StringBuilder()).append("onCreateView: id=0x").append(Integer.toHexString(j)).append(" fname=").append(s1).append(" existing=").append(s).toString());
                }
                if (s == null)
                {
                    context = Fragment.instantiate(context, s1);
                    ...
                    context.mFragmentManager = this;
                    context.onInflate(mActivity, attributeset, ((Fragment) (context)).mSavedFragmentState);
                    addFragment(context, true);
                } else
                {
                ...
                if (mCurState < 1 && ((Fragment) (context)).mFromLayout)
                {
                    moveToState(context, 1, 0, 0, false);
                } else
                {
                    moveToState(context);
                }
