...
class Logo
{
    ...
    public void draw(Canvas canvas, float f, float f1)
    {
        ...
        canvas.save();
        float f2 = (float)mLogo.getHeight() / (float)mLogo.getWidth();
        if (f1 / f < f2)
        {
            f = f1 / f2;
        } else
        ...
        f /= 2.0F;
        f1 /= 2.0F;
        canvas.drawBitmap(mLogo, new Rect(0, 0, mLogo.getWidth(), mLogo.getHeight()), new RectF(-f, -f1, f, f1), mPaint);
        canvas.restore();
    }

    void loadLogo(boolean flag)
    {
        if (mLogo != null && flag == mUseCardIOLogo)
        {
            return;
        }
        mUseCardIOLogo = flag;
