...
public class zzd
{
    ...
    private boolean a(PackageInfo packageinfo, boolean flag)
    {
        ...
        zzc.zzb zzb = new zzc.zzb(packageinfo.signatures[0].toByteArray());
        if (flag)
        {
            packageinfo = zzc.a();
        } else
        ...
        if (packageinfo.contains(zzb))
        {
            return true;
        }
        if (Log.isLoggable("GoogleSignatureVerifier", 2))
        {
            Log.v("GoogleSignatureVerifier", (new StringBuilder()).append("Signature not valid.  Found: \n").append(Base64.encodeToString(zzb.a(), 0)).toString());
        }
        return false;
    }

    transient zzc.zza a(PackageInfo packageinfo, zzc.zza azza[])
    {
        if (packageinfo.signatures.length != 1)
        {
            Log.w("GoogleSignatureVerifier", "Package has more than one signature.");
