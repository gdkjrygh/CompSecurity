...
public final class AppboyAdmReceiver extends BroadcastReceiver
{
    ...
    boolean handleRegistrationIntent(Context context, Intent intent)
    {
        ...
        if (s != null)
        {
            AppboyLogger.e(TAG, (new StringBuilder()).append("Error during ADM registration: ").append(s).toString());
        } else
        if (s1 != null)
        {
            AppboyLogger.i(TAG, (new StringBuilder()).append("Registering for ADM messages with registrationId: ").append(s1).toString());
            Appboy.getInstance(context).registerAppboyPushMessages(s1);
        } else
        if (intent != null)
        {
            AppboyLogger.i(TAG, (new StringBuilder()).append("Unregistering from ADM: ").append(intent).toString());
    ...
    private class HandleAppboyAdmMessageTask extends AsyncTask
    {
