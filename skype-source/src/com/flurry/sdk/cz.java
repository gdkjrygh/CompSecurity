...
public class cz
{
    ...
    public Map a(String s)
    {
        ...
        while (i < j) 
        {
            ...
            if (as1.length != 2)
            {
                eo.a(5, a, (new StringBuilder("Invalid referrer Element: ")).append(as[i]).append(" in referrer tag ").append(s).toString());
            } else
            {
        ...
        for (s = hashmap.entrySet().iterator(); s.hasNext(); eo.a(3, a, (new StringBuilder("entry: ")).append((String)entry.getKey()).append("=").append(entry.getValue()).toString()))
        {
        ...
        if (s.length() > 0)
        {
            eo.a(5, a, (new StringBuilder("Detected missing referrer keys : ")).append(s.toString()).toString());
        }
        return hashmap;
    }
