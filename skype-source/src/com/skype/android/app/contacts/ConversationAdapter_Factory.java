...
public final class ConversationAdapter_Factory
    implements Factory
{
    ...
    public ConversationAdapter_Factory(MembersInjector membersinjector, Provider provider, Provider provider1, Provider provider2, Provider provider3, Provider provider4, Provider provider5, 
    ...
    public static Factory create(MembersInjector membersinjector, Provider provider, Provider provider1, Provider provider2, Provider provider3, Provider provider4, Provider provider5, Provider provider6, 
            Provider provider7)
    {
        return new ConversationAdapter_Factory(membersinjector, provider, provider1, provider2, provider3, provider4, provider5, provider6, provider7);
    }

    public final ConversationAdapter get()
    {
        ConversationAdapter conversationadapter = new ConversationAdapter((Activity)contextProvider.get(), (ImageCache)imageCacheProvider.get(), (ContactUtil)contactUtilProvider.get(), (GroupAvatarUtil)groupAvatarUtilProvider.get(), (DefaultAvatars)defaultAvatarsProvider.get(), (SkyLib)libProvider.get(), (ObjectIdMap)mapProvider.get(), (TypeFaceFactory)typeFaceFactoryProvider.get());
        membersInjector.injectMembers(conversationadapter);
    ...
    {
        boolean flag;
        if (!com/skype/android/app/contacts/ConversationAdapter_Factory.desiredAssertionStatus())
        {
            flag = true;
        } else
        {
