...
//            MessagesSearchResultAdapter

public final class MessagesSearchResultAdapter_Factory
    implements Factory
{
    ...
    public MessagesSearchResultAdapter_Factory(MembersInjector membersinjector, Provider provider, Provider provider1, Provider provider2, Provider provider3, Provider provider4, Provider provider5, 
            Provider provider6)
    {
    ...
    {
        return new MessagesSearchResultAdapter_Factory(membersinjector, provider, provider1, provider2, provider3, provider4, provider5, provider6);
    }

    public final MessagesSearchResultAdapter get()
    {
        MessagesSearchResultAdapter messagessearchresultadapter = new MessagesSearchResultAdapter((Activity)activityProvider.get(), (ImageCache)imageCacheProvider.get(), (ContactUtil)contactUtilProvider.get(), (GroupAvatarUtil)groupAvatarUtilProvider.get(), (DefaultAvatars)defaultAvatarsProvider.get(), (SkyLib)libProvider.get(), (TypeFaceFactory)typeFaceFactoryProvider.get());
        membersInjector.injectMembers(messagessearchresultadapter);
        return messagessearchresultadapter;
    }
    ...
    {
        boolean flag;
        if (!com/skype/android/app/search/MessagesSearchResultAdapter_Factory.desiredAssertionStatus())
        {
            flag = true;
        } else
        {
