...
label0:
    ...
    protected void _addDeserializerConstructors(DeserializationContext deserializationcontext, BeanDescription beandescription, VisibilityChecker visibilitychecker, AnnotationIntrospector annotationintrospector, CreatorCollector creatorcollector)
    {
        ...
        do
        {
            ...
            int j1 = annotatedconstructor.getParameterCount();
            if (j1 < 1)
            {
                continue;
            }
            ...
            if (j1 == 1)
            {
                _handleSingleArgumentConstructor(deserializationcontext, beandescription, visibilitychecker, annotationintrospector, creatorcollector, annotatedconstructor, flag, flag1);
                continue;
            }
            if (!flag && !flag1)
            {
            ...
            while (i < j1) 
            {
                ...
                {
                    obj = annotationintrospector.findDeserializationName(annotatedparameter);
                }
                obj1 = annotationintrospector.findInjectableValueId(annotatedparameter);
                if (obj != null && ((String) (obj)).length() > 0)
                {
                    i1 = j + 1;
                    acreatorproperty[i] = constructCreatorProperty(deserializationcontext, beandescription, ((String) (obj)), i, annotatedparameter, obj1);
                    obj = s;
                    l = k;
                } else
                if (obj1 != null)
            ...
            if (flag || j > 0 || k > 0)
            {
                ...
                if (j == 0 && k + 1 == j1)
                {
                    creatorcollector.addDelegatingCreator(annotatedconstructor, acreatorproperty);
                } else
                {
                    throw new IllegalArgumentException((new StringBuilder()).append("Argument #").append(s.getIndex()).append(" of constructor ").append(annotatedconstructor).append(" has no property name annotation; must have name when multiple-paramater constructor annotated as Creator").toString());
                }
            }
        } while (true);
    }
    ...
    protected void _addDeserializerFactoryMethods(DeserializationContext deserializationcontext, BeanDescription beandescription, VisibilityChecker visibilitychecker, AnnotationIntrospector annotationintrospector, CreatorCollector creatorcollector)
    {
        ...
        do
        {
            ...
            AnnotatedParameter annotatedparameter1 = null;
            CreatorProperty acreatorproperty[] = new CreatorProperty[j1];
            int j = 0;
            int k = 0;
            int i = 0;
            while (i < j1) 
            {
                ...
                String s1 = annotationintrospector.findDeserializationName(annotatedparameter2);
                Object obj = annotationintrospector.findInjectableValueId(annotatedparameter2);
                int l;
                int i1;
                if (s1 != null && s1.length() > 0)
                {
                    i1 = j + 1;
                    acreatorproperty[i] = constructCreatorProperty(deserializationcontext, beandescription, s1, i, annotatedparameter2, obj);
                    annotatedparameter2 = annotatedparameter1;
                    l = k;
                } else
                if (obj != null)
                {
                ...
                i++;
                annotatedparameter1 = annotatedparameter2;
                k = l;
                j = i1;
            }
            if (flag || j > 0 || k > 0)
            {
                ...
                {
                    throw new IllegalArgumentException((new StringBuilder()).append("Argument #").append(annotatedparameter1.getIndex()).append(" of factory method ").append(annotatedmethod).append(" has no property name annotation; must have name when multiple-paramater constructor annotated as Creator").toString());
                }
            }
        } while (true);
    }
    ...
    protected ValueInstantiator _constructDefaultValueInstantiator(DeserializationContext deserializationcontext, BeanDescription beandescription)
    {
        ...
        CreatorCollector creatorcollector = new CreatorCollector(beandescription, flag);
        ...
        visibilitychecker = deserializationconfig.getAnnotationIntrospector().findAutoDetectVisibility(beandescription.getClassInfo(), visibilitychecker);
            ...
            boolean flag1)
    {
        ...
        String s = annotationintrospector.findDeserializationName(visibilitychecker);
        ...
        if (annotationintrospector != null || s != null && s.length() > 0)
        {
            creatorcollector.addPropertyCreator(annotatedconstructor, new CreatorProperty[] {
                constructCreatorProperty(deserializationcontext, beandescription, s, 0, visibilitychecker, annotationintrospector)
            });
            return true;
        }
...
_L6:
    ...
    public TypeDeserializer findPropertyContentTypeDeserializer(DeserializationConfig deserializationconfig, JavaType javatype, AnnotatedMember annotatedmember)
    {
        ...
        {
            return typeresolverbuilder.buildTypeDeserializer(deserializationconfig, javatype, deserializationconfig.getSubtypeResolver().collectAndResolveSubtypes(annotatedmember, deserializationconfig, annotationintrospector));
        }
    }
