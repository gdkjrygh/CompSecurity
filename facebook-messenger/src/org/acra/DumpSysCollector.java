...
class DumpSysCollector
{
    ...
    protected static String collectMemInfo(Context context)
    {
        ...
        stringbuilder.append((new StringBuilder()).append("avail/thresh/low? ").append(memoryinfo.availMem).append("/").append(memoryinfo.threshold).append("/").append(memoryinfo.lowMemory).append("/").append("(").append((int)((float)(100L * memoryinfo.threshold) / (float)memoryinfo.availMem)).append("%) memclass=").append(i).toString());
        ...
        stringbuilder.append(String.format(Locale.US, "          native: %7d / %7d / %7d", new Object[] {
            Integer.valueOf(memoryinfo1.dalvikPrivateDirty), Integer.valueOf(memoryinfo1.nativePss), Integer.valueOf(memoryinfo1.nativeSharedDirty)
        }));
