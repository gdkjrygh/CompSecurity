...
_L3:
    ...
    {
        certificate = certificate.getTBSCertificate().getExtensions();
        if (certificate != null)
        {
            certificate = certificate.getExtension(X509Extension.keyUsage);
            if (certificate != null && (KeyUsage.getInstance(certificate).getBytes()[0] & 0xff & i) != i)
            {
