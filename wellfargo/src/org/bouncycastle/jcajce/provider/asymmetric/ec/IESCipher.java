...
public class IESCipher extends CipherSpi
{
    ...
    public byte[] engineDoFinal(byte abyte0[], int i, int j)
    {
        ...
        Object obj = ((ECKeyParameters)key).getParameters();
        ...
        if (state == 1 || state == 3)
        {
            ...
            eckeypairgenerator.init(new ECKeyGenerationParameters(((org.bouncycastle.crypto.params.ECDomainParameters) (obj)), random));
        ...
        if (state == 2 || state == 4)
        {
            try
            {
                engine.init(key, ieswithcipherparameters, new ECIESPublicKeyParser(((org.bouncycastle.crypto.params.ECDomainParameters) (obj))));
                abyte0 = engine.processBlock(abyte0, 0, abyte0.length);
            }
            // Misplaced declaration of an exception variable
            catch (byte abyte0[])
    ...
    {
        if (key1 instanceof ECKey)
        {
            return ((ECKey)key1).getParameters().getCurve().getFieldSize();
        } else
        {
    ...
    public int engineGetOutputSize(int i)
    {
        ...
        if (key != null)
        {
            k = ((((ECKey)key).getParameters().getCurve().getFieldSize() + 7) * 2) / 8 + 1;
            if (engine.getCipher() != null)
            {
                if (state == 1 || state == 3)
                {
    ...
    public void engineInit(int i, Key key1, AlgorithmParameterSpec algorithmparameterspec, SecureRandom securerandom)
    {
        ...
        if (i == 1 || i == 3)
        {
            if (key1 instanceof ECPublicKey)
            {
                key = ECUtil.generatePublicKeyParameter((PublicKey)key1);
            } else
            if (key1 instanceof IESKey)
            {
                key1 = (IESKey)key1;
                key = ECUtil.generatePublicKeyParameter(key1.getPublic());
                otherKeyParameter = ECUtil.generatePrivateKeyParameter(key1.getPrivate());
            } else
            {
        ...
        if (i == 2 || i == 4)
        {
            if (key1 instanceof ECPrivateKey)
            {
                key = ECUtil.generatePrivateKeyParameter((PrivateKey)key1);
            } else
            if (key1 instanceof IESKey)
            {
                key1 = (IESKey)key1;
                otherKeyParameter = ECUtil.generatePublicKeyParameter(key1.getPublic());
                key = ECUtil.generatePrivateKeyParameter(key1.getPrivate());
            } else
            {
