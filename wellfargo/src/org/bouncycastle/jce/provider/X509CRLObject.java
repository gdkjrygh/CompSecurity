...
_L1:
    ...
    static boolean isIndirectCRL(X509CRL x509crl)
    {
        ...
        flag = IssuingDistributionPoint.getInstance(ASN1OctetString.getInstance(x509crl).getOctets()).isIndirectCRL();
        if (flag)
        {
            return true;
        }
...
_L3:
        ...
        {
            stringbuffer.append(new CRLNumber(ASN1Integer.getInstance(asn1inputstream.readObject()).getPositiveValue())).append(s);
            continue; /* Loop/switch isn't completed */
        }
        if (asn1objectidentifier.equals(Extension.deltaCRLIndicator))
        {
            stringbuffer.append((new StringBuilder()).append("Base CRL: ").append(new CRLNumber(ASN1Integer.getInstance(asn1inputstream.readObject()).getPositiveValue())).toString()).append(s);
            continue; /* Loop/switch isn't completed */
        }
