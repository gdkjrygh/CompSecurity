...
_L8:
    ...
    private static void deleteFile(File file)
    {
        ...
        String s = ACRA.LOG_TAG;
        (new StringBuilder("Could not delete error report: ")).append(file.getName());
    }
...
_L1:
    ...
    private CrashReportData loadCrashReport(Context context, File file, CrashReportType crashreporttype, long l)
    {
        ...
        if (s.endsWith(".temp_stacktrace"))
        {
            ...
            {
                context = ACRA.LOG_TAG;
                (new StringBuilder("temp file ")).append(s).append(" is too recent; skipping");
                return null;
            }
            if (!file.exists())
            {
                context = ACRA.LOG_TAG;
                (new StringBuilder("temp file ")).append(s).append(" deleted. skipping");
                return null;
            }
        }
        crashreportdata = new CrashReportData();
        ...
        {
            context = ACRA.LOG_TAG;
            (new StringBuilder("crash report ")).append(s).append(" was too old; deleted");
            deleteFile(file);
        ...
        {
            context = ACRA.LOG_TAG;
            (new StringBuilder()).append(l1).append("-byte crash report ").append(s).append(" exceeded max size of ").append(l).append(" bytes; deleted");
            deleteFile(file);
            if (crashreporttype.attachmentField != null)
            {
...
_L22:
        ...
        // Misplaced declaration of an exception variable
          ...
          goto _L20
        obj;
        AndroidCompat.a(crashreporttype, ((Throwable) (obj)));
          goto _L21
_L17:
        bufferedreader.close();
          goto _L21
...
_L2:
        ...
        String s = ACRA.LOG_TAG;
        (new StringBuilder("ReportSender of class ")).append(reportsender.getClass().getName()).append(" failed but other senders completed their task. ACRA will not send this report again.");
        if (true) goto _L2; else goto _L1
_L1:
    }
