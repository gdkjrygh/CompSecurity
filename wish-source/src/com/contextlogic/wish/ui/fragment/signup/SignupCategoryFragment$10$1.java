// Decompiled by Jad v1.5.8e. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.geocities.com/kpdus/jad.html
// Decompiler options: braces fieldsfirst space lnc 

package com.contextlogic.wish.ui.fragment.signup;


// Referenced classes of package com.contextlogic.wish.ui.fragment.signup:
//            SignupCategoryFragment

class this._cls1
    implements Runnable
{

    final is._cls0 this$1;

    public void run()
    {
        SignupCategoryFragment.access$600(_fld0);
    }

    is._cls0()
    {
        this$1 = this._cls1.this;
        super();
    }

    // Unreferenced inner class com/contextlogic/wish/ui/fragment/signup/SignupCategoryFragment$10

/* anonymous class */
    class SignupCategoryFragment._cls10
        implements com.contextlogic.wish.api.service.ApiService.FailureCallback
    {

        final SignupCategoryFragment this$0;

        public void onServiceFailed()
        {
            postDelayed(new SignupCategoryFragment._cls10._cls1(), getAnimationTimeRemaining());
        }

            
            {
                this$0 = SignupCategoryFragment.this;
                super();
            }
    }

}
