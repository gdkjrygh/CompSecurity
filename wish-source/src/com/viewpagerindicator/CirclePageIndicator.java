    ...
    implements PageIndicator
{
    ...
    private int measureLong(int i)
    {
        ...
        } else
        {
            ...
            k = (int)((float)(getPaddingLeft() + getPaddingRight()) + (float)(i * 2) * mRadius + (float)(i - 1) * mRadius + 1.0F);
            i = k;
            if (l == 0x80000000)
            {
                return Math.min(k, j);
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        if (mViewPager != null)
        {
            ...
            if (i != 0)
            {
                if (mForceCount == -1 && mCurrentPage >= i)
                {
                    setCurrentItem(i - 1);
                    return;
                }
                ...
                if (mCentered)
                {
                    f1 = f2 + ((float)(i1 - l - k) / 2.0F - ((float)i * f8) / 2.0F);
                }
                f4 = mRadius;
                f2 = f4;
                if (mPaintStroke.getStrokeWidth() > 0.0F)
                {
                    f2 = f4 - mPaintStroke.getStrokeWidth() / 2.0F;
                }
                k = 0;
                while (k < i) 
                {
                    float f7 = f1 + (float)k * f8;
                    float f5;
                    if (mOrientation == 0)
                    {
                        f5 = f7;
                    ...
                    {
                        canvas.drawCircle(f5, f7, f2, mPaintPageFill);
                    }
                    if (f2 != mRadius)
                    {
                        canvas.drawCircle(f5, f7, mRadius, mPaintStroke);
                    }
                    k++;
                }
                if (mCurrentPage <= i)
...
_L7:
    ...
    public void setForceCount(int i)
    {
        mForceCount = i;
    }
