    ...
    implements BlockCipher
{
    ...
    private int bytes2int(byte abyte0[], int l)
    {
        int i1 = 0;
        int j1 = 0;
        for (; i1 < 4; i1++)
        {
            j1 = (j1 << 8) + (abyte0[i1 + l] & 0xff);
        }

        return j1;
    }

    private void camelliaF2(int ai[], int ai1[], int l)
    {
        int i1 = ai[0] ^ ai1[l + 0];
        int j1 = i[i1 & 0xff];
        int k1 = k[i1 >>> 8 & 0xff];
        int l1 = j[i1 >>> 16 & 0xff];
        i1 = h[i1 >>> 24 & 0xff] ^ (j1 ^ k1 ^ l1);
        j1 = ai[1] ^ ai1[l + 1];
        k1 = h[j1 & 0xff];
        l1 = i[j1 >>> 8 & 0xff];
        int i2 = k[j1 >>> 16 & 0xff];
        j1 = j[j1 >>> 24 & 0xff] ^ (k1 ^ l1 ^ i2);
        ai[2] = ai[2] ^ (i1 ^ j1);
        k1 = ai[3];
        ai[3] = rightRotate(i1, 8) ^ (j1 ^ i1) ^ k1;
        i1 = ai[2] ^ ai1[l + 2];
        j1 = i[i1 & 0xff];
        k1 = k[i1 >>> 8 & 0xff];
        l1 = j[i1 >>> 16 & 0xff];
        ...
        k1 = i[l >>> 8 & 0xff];
        l1 = k[l >>> 16 & 0xff];
        l = j[l >>> 24 & 0xff] ^ (j1 ^ k1 ^ l1);
        ai[0] = ai[0] ^ (i1 ^ l);
        j1 = ai[1];
        ai[1] = rightRotate(i1, 8) ^ (l ^ i1) ^ j1;
    }

    private void camelliaFLs(int ai[], int ai1[], int l)
    {
        ai[1] = ai[1] ^ leftRotate(ai[0] & ai1[l + 0], 1);
        ai[0] = ai[0] ^ (ai1[l + 1] | ai[1]);
        ai[2] = ai[2] ^ (ai1[l + 3] | ai[3]);
        ai[3] = ai[3] ^ leftRotate(ai1[l + 2] & ai[2], 1);
    }

    private static void decroldq(int l, int ai[], int i1, int ai1[], int j1)
    {
        ai1[j1 + 2] = ai[i1 + 0] << l | ai[i1 + 1] >>> 32 - l;
        ai1[j1 + 3] = ai[i1 + 1] << l | ai[i1 + 2] >>> 32 - l;
        ai1[j1 + 0] = ai[i1 + 2] << l | ai[i1 + 3] >>> 32 - l;
        ai1[j1 + 1] = ai[i1 + 3] << l | ai[i1 + 0] >>> 32 - l;
        ai[i1 + 0] = ai1[j1 + 2];
        ai[i1 + 1] = ai1[j1 + 3];
        ai[i1 + 2] = ai1[j1 + 0];
        ai[i1 + 3] = ai1[j1 + 1];
    ...
    private static void decroldqo32(int l, int ai[], int i1, int ai1[], int j1)
    {
        ai1[j1 + 2] = ai[i1 + 1] << l - 32 | ai[i1 + 2] >>> 64 - l;
        ai1[j1 + 3] = ai[i1 + 2] << l - 32 | ai[i1 + 3] >>> 64 - l;
        ai1[j1 + 0] = ai[i1 + 3] << l - 32 | ai[i1 + 0] >>> 64 - l;
        ai1[j1 + 1] = ai[i1 + 0] << l - 32 | ai[i1 + 1] >>> 64 - l;
        ai[i1 + 0] = ai1[j1 + 2];
        ai[i1 + 1] = ai1[j1 + 3];
        ai[i1 + 2] = ai1[j1 + 0];
        ai[i1 + 3] = ai1[j1 + 1];
    ...
    private void int2bytes(int l, byte abyte0[], int i1)
    {
        boolean flag = false;
        int j1 = l;
        for (l = ((flag) ? 1 : 0); l < 4; l++)
        {
            abyte0[(3 - l) + i1] = (byte)j1;
            j1 >>>= 8;
        }

    }

    private static int leftRotate(int l, int i1)
    {
        return (l << i1) + (l >>> 32 - i1);
    }

    private int processBlock128(byte abyte0[], int l, byte abyte1[], int i1)
    {
        for (int j1 = 0; j1 < 4; j1++)
        {
            f[j1] = bytes2int(abyte0, j1 * 4 + l);
            int ai[] = f;
            ai[j1] = ai[j1] ^ d[j1];
        }
        ...
        int2bytes(f[2], abyte1, i1);
        int2bytes(f[3], abyte1, i1 + 4);
        int2bytes(f[0], abyte1, i1 + 8);
        int2bytes(f[1], abyte1, i1 + 12);
        return 16;
    }

    private int processBlock192or256(byte abyte0[], int l, byte abyte1[], int i1)
    {
        for (int j1 = 0; j1 < 4; j1++)
        {
            f[j1] = bytes2int(abyte0, j1 * 4 + l);
    ...
    private static int rightRotate(int l, int i1)
    {
        return (l >>> i1) + (l << 32 - i1);
    }

    private static void roldq(int l, int ai[], int i1, int ai1[], int j1)
    {
        ai1[j1 + 0] = ai[i1 + 0] << l | ai[i1 + 1] >>> 32 - l;
        ai1[j1 + 1] = ai[i1 + 1] << l | ai[i1 + 2] >>> 32 - l;
        ai1[j1 + 2] = ai[i1 + 2] << l | ai[i1 + 3] >>> 32 - l;
        ai1[j1 + 3] = ai[i1 + 3] << l | ai[i1 + 0] >>> 32 - l;
        ai[i1 + 0] = ai1[j1 + 0];
        ai[i1 + 1] = ai1[j1 + 1];
        ai[i1 + 2] = ai1[j1 + 2];
        ai[i1 + 3] = ai1[j1 + 3];
    ...
    private static void roldqo32(int l, int ai[], int i1, int ai1[], int j1)
    {
        ai1[j1 + 0] = ai[i1 + 1] << l - 32 | ai[i1 + 2] >>> 64 - l;
        ai1[j1 + 1] = ai[i1 + 2] << l - 32 | ai[i1 + 3] >>> 64 - l;
        ai1[j1 + 2] = ai[i1 + 3] << l - 32 | ai[i1 + 0] >>> 64 - l;
        ai1[j1 + 3] = ai[i1 + 0] << l - 32 | ai[i1 + 1] >>> 64 - l;
        ai[i1 + 0] = ai1[j1 + 0];
        ai[i1 + 1] = ai1[j1 + 1];
        ai[i1 + 2] = ai1[j1 + 2];
        ai[i1 + 3] = ai1[j1 + 3];
...
_L5:
    ...
    public int processBlock(byte abyte0[], int l, byte abyte1[], int i1)
    {
        if (!a)
        {
            throw new IllegalStateException("Camellia engine not initialised");
        }
        if (l + 16 > abyte0.length)
        {
            throw new DataLengthException("input buffer too short");
        }
        if (i1 + 16 > abyte1.length)
        {
            throw new DataLengthException("output buffer too short");
        }
        if (b)
        {
            return processBlock128(abyte0, l, abyte1, i1);
        } else
        {
            return processBlock192or256(abyte0, l, abyte1, i1);
        }
    }
