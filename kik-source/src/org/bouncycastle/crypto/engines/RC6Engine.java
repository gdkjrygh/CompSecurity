    ...
    implements BlockCipher
{
    ...
    private int decryptBlock(byte abyte0[], int i, byte abyte1[], int j)
    {
        ...
        int j1 = bytesToWord(abyte0, i + 8);
        int l = bytesToWord(abyte0, i + 12);
        i = j1 - a[43];
        k -= a[42];
        for (j1 = 20; j1 > 0;)
        {
            int k1 = rotateLeft((k * 2 + 1) * k, 5);
            int i2 = rotateLeft((i * 2 + 1) * i, 5);
            i1 = rotateRight(i1 - a[j1 * 2 + 1], k1);
            l = rotateRight(l - a[j1 * 2], i2);
            j1--;
            k1 = l ^ k1;
            l = i;
            i = i1 ^ i2;
        ...
        j1 = a[1];
        int l1 = a[0];
        wordToBytes(k, abyte1, j);
        wordToBytes(i1 - l1, abyte1, j + 4);
        wordToBytes(i, abyte1, j + 8);
        wordToBytes(l - j1, abyte1, j + 12);
        return 16;
    }
    ...
    private int encryptBlock(byte abyte0[], int i, byte abyte1[], int j)
    {
        ...
        while (i1 <= 20) 
        {
            ...
            int j2 = rotateLeft((i * 2 + 1) * i, 5);
            k1 = rotateLeft(j1 ^ i2, j2);
            int l1 = a[i1 * 2];
            l = rotateLeft(l ^ j2, i2);
            i2 = a[i1 * 2 + 1];
            i1++;
            j1 = k;
            k = l + i2;
        ...
        i1 = a[42];
        k1 = a[43];
        wordToBytes(j1 + i1, abyte1, j);
        wordToBytes(k, abyte1, j + 4);
        wordToBytes(l + k1, abyte1, j + 8);
