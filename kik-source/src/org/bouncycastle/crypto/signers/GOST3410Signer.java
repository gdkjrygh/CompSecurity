    ...
    implements DSA
{
    ...
    public BigInteger[] generateSignature(byte abyte0[])
    {
        ...
        GOST3410Parameters gost3410parameters = a.getParameters();
        BigInteger biginteger;
        do
        {
            biginteger = new BigInteger(gost3410parameters.getQ().bitLength(), b);
        ...
        return (new BigInteger[] {
            biginteger1, biginteger.multiply(abyte0).add(((GOST3410PrivateKeyParameters)a).getX().multiply(biginteger1)).mod(gost3410parameters.getQ())
        });
    }
    ...
    public void init(boolean flag, CipherParameters cipherparameters)
    {
        if (flag)
        {
            if (cipherparameters instanceof ParametersWithRandom)
            {
                ...
                a = (GOST3410PrivateKeyParameters)cipherparameters.getParameters();
                return;
            } else
            {
                b = new SecureRandom();
    ...
    public boolean verifySignature(byte abyte0[], BigInteger biginteger, BigInteger biginteger1)
    {
        ...
        return abyte0.getA().modPow(biginteger1, abyte0.getP()).multiply(((GOST3410PublicKeyParameters)a).getY().modPow(biginteger2, abyte0.getP())).mod(abyte0.getP()).mod(abyte0.getQ()).equals(biginteger);
    }
}
