...
_L3:
    ...
    private int getSpanGroupIndex(RecyclerView.Recycler recycler, RecyclerView.State state, int i)
    {
        ...
        i = recycler.convertPreLayoutPositionToPostLayout(i);
        if (i == -1)
        {
            return 0;
        } else
...
label1:
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LinearLayoutManager.LayoutState layoutstate, LinearLayoutManager.LayoutChunkResult layoutchunkresult)
    {
        ...
        while (j < i1) 
        {
            ...
            if (mOrientation == 1)
            {
                measureChildWithDecorationsAndMargin(recycler, k, getMainDirSpec(((LayoutParams) (state)).height));
            } else
            {
                measureChildWithDecorationsAndMargin(recycler, getMainDirSpec(((LayoutParams) (state)).width), k);
            }
        ...
        while (j < i1) 
        {
            ...
            if (mOrientationHelper.getDecoratedMeasurement(recycler) != i)
            {
                ...
                if (mOrientation == 1)
                {
                    measureChildWithDecorationsAndMargin(recycler, l, k);
                } else
                {
                    measureChildWithDecorationsAndMargin(recycler, k, l);
                }
            }
            j++;
        }
