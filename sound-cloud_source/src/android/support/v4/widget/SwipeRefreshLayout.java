...
_L8:
        mInitialDownY = f;
          goto _L9
_L6:
        if (mActivePointerId == -1) goto _L1; else goto _L10
_L10:
        f = getMotionEventY(motionevent, mActivePointerId);
        if (f == -1F) goto _L1; else goto _L11
_L11:
...
_L4:
        ...
        float f2 = 0.5F * (MotionEventCompat.getY(motionevent, j) - mInitialMotionY);
        if (!mIsBeingDragged)
        {
            continue; /* Loop/switch isn't completed */
        }
...
_L9:
        ...
        setTargetOffsetTopAndBottom((k + j) - mCurrentTargetOffsetTop, true);
        continue; /* Loop/switch isn't completed */
_L8:
