    ...
    implements BlockCipher
{
    ...
    private long F(int i, int j, long l)
    {
        int k = (int)(l >> 32);
        int i1 = (int)l;
        int j1 = phaseCalc2(k, i, i1, j);
        return (long)(phaseCalc1(k, i, i1, j) + j1) << 32 | (long)j1 & 0xffffffffL;
    }

    private int G(int i)
    {
        return SS0[i & 0xff] ^ SS1[i >> 8 & 0xff] ^ SS2[i >> 16 & 0xff] ^ SS3[i >> 24 & 0xff];
    }

    private long bytesToLong(byte abyte0[], int i)
    {
        long l = 0L;
        for (int j = 0; j <= 7; j++)
        {
    ...
    private int[] createWorkingKey(byte abyte0[])
    {
        int ai[] = new int[32];
        long l1 = bytesToLong(abyte0, 0);
        long l2 = bytesToLong(abyte0, 8);
        int j = extractW0(l1);
        int k = extractW1(l1);
        int i1 = extractW0(l2);
        int l = extractW1(l2);
    ...
    private void longToBytes(byte abyte0[], int i, long l)
    {
        for (int j = 0; j < 8; j++)
        {
            abyte0[j + i] = (byte)(int)(l >> (7 - j) * 8);
        }

    }

    private int phaseCalc1(int i, int j, int k, int l)
    {
        return G(G(i ^ j ^ (k ^ l)) + (i ^ j));
    }

    private int phaseCalc2(int i, int j, int k, int l)
    {
        return G(phaseCalc1(i, j, k, l) + G(i ^ j ^ (k ^ l)));
    }
    ...
    {
        forEncryption = flag;
        wKey = createWorkingKey(((KeyParameter)cipherparameters).getKey());
    }
        ...
        throws DataLengthException, IllegalStateException
    {
        ...
        long l = bytesToLong(abyte0, i + 0);
        long l3 = bytesToLong(abyte0, i + 8);
        long l4;
        if (forEncryption)
        {
            i = 0;
        ...
        longToBytes(abyte1, j + 0, l3);
        longToBytes(abyte1, j + 8, l4);
        return 16;
    }
