    ...
    implements BlockCipher
{
    ...
    private int F(int i)
    {
        return (S0[i >>> 24] + S1[i >>> 16 & 0xff] ^ S2[i >>> 8 & 0xff]) + S3[i & 0xff];
    }

    private void decryptBlock(byte abyte0[], int i, byte abyte1[], int j)
    {
        int k = BytesTo32bits(abyte0, i);
        int i1 = BytesTo32bits(abyte0, i + 4);
        i = P[17] ^ k;
        for (int l = 16; l > 0; l -= 2)
        {
            i1 ^= F(i) ^ P[l];
            i ^= F(i1) ^ P[l - 1];
        }

        Bits32ToBytes(P[0] ^ i1, abyte1, j);
        Bits32ToBytes(i, abyte1, j + 4);
    }
    ...
    private void encryptBlock(byte abyte0[], int i, byte abyte1[], int j)
    {
        ...
        Bits32ToBytes(P[17] ^ i1, abyte1, j);
        Bits32ToBytes(i, abyte1, j + 4);
    }

    private void processTable(int i, int j, int ai[])
    {
        int i1 = ai.length;
        for (int k = 0; k < i1;)
        {
    ...
    public void init(boolean flag, CipherParameters cipherparameters)
    {
        ...
        {
            throw new IllegalArgumentException((new StringBuilder()).append("invalid parameter passed to Blowfish init - ").append(cipherparameters.getClass().getName()).toString());
        }
    }
