...
public class BufferedEncoder
{
    ...
    public BufferedEncoder(Translator translator1, int i)
    {
    ...
    public int processByte(byte byte0, byte abyte0[], int i)
    {
        ...
        {
            j = translator.encode(buf, 0, buf.length, abyte0, i);
            bufOff = 0;
        }
        return j;
    }
    ...
    public int processBytes(byte abyte0[], int i, int j, byte abyte1[], int k)
    {
        ...
        if (j > k1)
        {
            ...
            j1 = 0 + translator.encode(buf, 0, buf.length, abyte1, k);
            bufOff = 0;
            j -= k1;
            i += k1;
            i1 = j - j % buf.length;
            k = translator.encode(abyte0, i, i1, abyte1, k + j1);
            l = j - i1;
            i1 = i + i1;
            j1 = k + j1;
        }
