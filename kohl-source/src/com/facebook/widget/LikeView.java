...
_L4:
    ...
    private void broadcastIfUserLiked()
    {
        if (likeActionController != null)
        {
            boolean flag = likeActionController.isObjectLiked();
            Intent intent = new Intent("com.kohls.fb.isLiked");
            intent.putExtra("isLiked", flag);
    ...
    private void parseAttributes(AttributeSet attributeset)
    {
        if (attributeset != null && getContext() != null)
        {
            ...
            {
                objectId = Utility.coerceValueIfNullOrEmpty(attributeset.getString(1), null);
                likeViewStyle = Style.fromInt(attributeset.getInt(2, Style.DEFAULT.getValue()));
                if (likeViewStyle == null)
                {
                    throw new IllegalArgumentException("Unsupported value for LikeView 'style'");
                }
                auxiliaryViewPosition = AuxiliaryViewPosition.fromInt(attributeset.getInt(3, AuxiliaryViewPosition.DEFAULT.getValue()));
                if (auxiliaryViewPosition == null)
                {
                    throw new IllegalArgumentException("Unsupported value for LikeView 'auxiliary_view_position'");
                }
                horizontalAlignment = HorizontalAlignment.fromInt(attributeset.getInt(4, HorizontalAlignment.DEFAULT.getValue()));
                if (horizontalAlignment == null)
                {
                    throw new IllegalArgumentException("Unsupported value for LikeView 'horizontal_alignment'");
                } else
                {
                    foregroundColor = attributeset.getColor(0, -1);
...
_L3:
        ...
        if (!(context instanceof Activity)) goto _L6; else goto _L5
_L5:
