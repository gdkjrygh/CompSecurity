...
public class StickyGridHeadersBaseAdapterWrapper extends BaseAdapter
{
    ...
    {
        viewgroup = null;
        if (null instanceof FillerView)
        {
            viewgroup = (FillerView)view;
        }
        view = viewgroup;
        if (viewgroup == null)
        {
            view = new FillerView(mContext);
        }
    ...
    {
        viewgroup = (HeaderFillerView)view;
        view = viewgroup;
        if (viewgroup == null)
        {
            view = new HeaderFillerView(mContext);
        }
        return view;
    }
    ...
    private int unFilledSpacesInHeaderGroup(int i)
    {
        if (mNumColumns != 0)
        {
            if ((i = mDelegate.getCountForHeader(i) % mNumColumns) != 0)
            {
    ...
    public View getView(int i, View view, ViewGroup viewgroup)
    {
        ...
        if (position.mPosition == -1)
        {
            return getFillerView(view, viewgroup, mLastViewSeen);
        } else
        {
