...
public class SnowAlarm extends AbstractAlarm
{
    ...
    private static double getSnowThreshold(Integer integer)
    {
        return integer.intValue() != 0 ? 2.54D : 1.0D;
    }
    ...
    public boolean hasAlarm(ForecastValues forecastvalues, Integer integer, int i)
    {
        ...
        {
            flag = flag1;
            if (Double.parseDouble(forecastvalues.getSnowForAlarm()) > getSnowThreshold(integer))
            {
                flag = true;
            }
        }
