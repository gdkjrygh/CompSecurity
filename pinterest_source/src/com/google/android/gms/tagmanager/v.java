    ...
    implements DataLayer.c
{
    ...
    private void cA(String s)
    {
        ...
        bh.V((new StringBuilder("Cleared ")).append(k).append(" items").toString());
        pC();
        return;
        SQLiteException sqliteexception;
        sqliteexception;
        bh.W((new StringBuilder("Error deleting entries with key prefix: ")).append(s).append(" (").append(sqliteexception).append(").").toString());
        pC();
        return;
        s;
        pC();
    ...
    private void fI(int k)
    {
        ...
        {
            List list = fJ(k);
            bh.U((new StringBuilder("DataLayer store full, deleting ")).append(list.size()).append(" entries to make room.").toString());
...
_L6:
        obj = obj1;
        bh.W((new StringBuilder("Error in peekEntries fetching entryIds: ")).append(sqliteexception.getMessage()).toString());
        if (obj1 != null)
        {
            ((Cursor) (obj1)).close();
        }
    ...
    private void h(String as[])
    {
        ...
        if (as != null && as.length != 0)
        {
            if ((sqlitedatabase = al("Error opening database for deleteEntries.")) != null)
            {
                ...
                {
                    bh.W((new StringBuilder("Error deleting entries ")).append(Arrays.toString(as)).toString());
                }
                return;
            }
        }
...
_L4:
    ...
    private void x(long l)
    {
        ...
        try
        {
            ...
            bh.V((new StringBuilder("Deleted ")).append(k).append(" expired items").toString());
            return;
        }
        catch (SQLiteException sqliteexception)
        {
