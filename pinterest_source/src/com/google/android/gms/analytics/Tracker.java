...
public class Tracker
{
    ...
    void a(am am1)
    {
        ...
        if (Cm.ft())
        {
            am1 = Cm.fu();
            set("&tid", am1);
            ae.V((new StringBuilder("[Tracker] trackingId loaded: ")).append(am1).toString());
        }
        if (Cm.fv())
        {
            am1 = Double.toString(Cm.fw());
            set("&sf", am1);
            ae.V((new StringBuilder("[Tracker] sample frequency loaded: ")).append(am1).toString());
        }
        if (Cm.fx())
        {
            setSessionTimeout(Cm.getSessionTimeout());
            ae.V((new StringBuilder("[Tracker] session timeout loaded: ")).append(fn()).toString());
        }
        if (Cm.fy())
        {
            enableAutoActivityTracking(Cm.fz());
            ae.V((new StringBuilder("[Tracker] auto activity tracking loaded: ")).append(fo()).toString());
        }
        if (Cm.fA())
        {
            if (Cm.fB())
