...
class a extends BackgroundTask
{
    ...
    public void run()
    {
        c = PinFeed.parseJson(a, cess._mth800(b));
        if (c == null)
        {
            CrashReporting.a(new NullPointerException((new StringBuilder("Failed to construct PinFeed response [")).append(a).append("] baseUrl [").append(cess._mth900(b)).append("] with streaming parser").toString()));
            return;
        }
        ...
        try
        {
            Preferences.user().set("PREF_HOME_FEED_STREAMING", true);
            DiskCache.setJsonString("MY_HOME_FEED", a);
            return;
        }
