...
label0:
    ...
    private void processAdapterUpdatesAndSetAnimationFlags()
    {
        ...
        State.access._mth2302(state, flag1);
        state = mState;
        if (State.access._mth2300(mState) && flag && !mDataSetHasChangedAfterLayout && predictiveItemAnimationsEnabled())
        {
            flag1 = flag2;
        } else
        {
        ...
        State.access._mth2102(state, flag1);
    }
...
_L5:
    ...
    private void recordAnimationInfoIfBouncedHiddenView(ViewHolder viewholder, ItemAnimator.ItemHolderInfo itemholderinfo)
    {
        ...
        if (State.access._mth2500(mState) && viewholder.isUpdated() && !viewholder.isRemoved() && !viewholder.shouldIgnore())
        {
    ...
    private void setAdapterInternal(Adapter adapter, boolean flag, boolean flag1)
    {
        ...
        State.access._mth1902(mState, true);
        markKnownViewsInvalid();
    }
    ...
    void dispatchLayout()
    {
        ...
        } else
        {
            ...
            Object obj = mState;
            boolean flag;
            if (State.access._mth2300(mState) && mItemsChanged)
            {
                flag = true;
            } else
            {
            ...
            State.access._mth2502(((State) (obj)), flag);
            mItemsChanged = false;
            mItemsAddedOrRemoved = false;
            State.access._mth2202(mState, State.access._mth2100(mState));
            mState.mItemCount = mAdapter.getItemCount();
            findMinMaxChildLayoutPositions(mMinMaxLayoutPositions);
            if (State.access._mth2300(mState))
            {
                ...
                for (int i = 0; i < l; i++)
                {
                    ...
                    ItemAnimator.ItemHolderInfo itemholderinfo = mItemAnimator.recordPreLayoutInformation(mState, ((ViewHolder) (obj)), ItemAnimator.buildAdapterChangeFlagsForAnimations(((ViewHolder) (obj))), ((ViewHolder) (obj)).getUnmodifiedPayloads());
                    mViewInfoStore.addToPreLayout(((ViewHolder) (obj)), itemholderinfo);
                    if (State.access._mth2500(mState) && ((ViewHolder) (obj)).isUpdated() && !((ViewHolder) (obj)).isRemoved() && !((ViewHolder) (obj)).shouldIgnore() && !((ViewHolder) (obj)).isInvalid())
                    {
                        long l1 = getChangedHolderKey(((ViewHolder) (obj)));
                        mViewInfoStore.addToOldChangeHolders(l1, ((ViewHolder) (obj)));
                    }
                }

            }
            if (State.access._mth2100(mState))
            {
                saveOldPositions();
                flag = State.access._mth1900(mState);
                State.access._mth1902(mState, false);
                mLayout.onLayoutChildren(mRecycler, mState);
                State.access._mth1902(mState, flag);
                int j = 0;
                while (j < mChildHelper.getChildCount()) 
                {
                    obj = getChildViewHolderInt(mChildHelper.getChildAt(j));
                    if (!((ViewHolder) (obj)).shouldIgnore() && !mViewInfoStore.isInPreLayout(((ViewHolder) (obj))))
                    {
                        int k1 = ItemAnimator.buildAdapterChangeFlagsForAnimations(((ViewHolder) (obj)));
                        flag = ((ViewHolder) (obj)).hasAnyOfTheFlags(8192);
                        int i1 = k1;
                        if (!flag)
                        {
                            i1 = k1 | 0x1000;
                        }
                        ItemAnimator.ItemHolderInfo itemholderinfo1 = mItemAnimator.recordPreLayoutInformation(mState, ((ViewHolder) (obj)), i1, ((ViewHolder) (obj)).getUnmodifiedPayloads());
                        if (flag)
                        {
                            recordAnimationInfoIfBouncedHiddenView(((ViewHolder) (obj)), itemholderinfo1);
                        } else
                        {
                            mViewInfoStore.addToAppearedInPreLayoutHolders(((ViewHolder) (obj)), itemholderinfo1);
                        }
                    }
                    j++;
                }
            ...
            mState.mItemCount = mAdapter.getItemCount();
            State.access._mth1802(mState, 0);
            State.access._mth2202(mState, false);
            mLayout.onLayoutChildren(mRecycler, mState);
            State.access._mth1902(mState, false);
            mPendingSavedState = null;
            obj = mState;
            if (State.access._mth2300(mState) && mItemAnimator != null)
            {
                flag = true;
            } else
            {
            ...
            State.access._mth2302(((State) (obj)), flag);
            if (State.access._mth2300(mState))
            {
                ...
                while (k < j1) 
                {
                    ...
                    if (!viewholder.shouldIgnore())
                    {
                        ...
                        ItemAnimator.ItemHolderInfo itemholderinfo2 = mItemAnimator.recordPostLayoutInformation(mState, viewholder);
                        ViewHolder viewholder1 = mViewInfoStore.getFromOldChangeHolders(l2);
                        if (viewholder1 != null && !viewholder1.shouldIgnore())
                        {
                            animateChange(viewholder1, viewholder, mViewInfoStore.popFromPreLayout(viewholder1), itemholderinfo2);
                        } else
                        {
                            mViewInfoStore.addToPostLayout(viewholder, itemholderinfo2);
                        }
                    }
                    k++;
                }
            ...
            resumeRequestLayout(false);
            mLayout.removeAndRecycleScrapInt(mRecycler);
            State.access._mth2602(mState, mState.mItemCount);
            mDataSetHasChangedAfterLayout = false;
            State.access._mth2302(mState, false);
            State.access._mth2102(mState, false);
            onExitLayoutOrScroll();
            mLayout.mRequestedSimpleAnimations = false;
            if (Recycler.access._mth2700(mRecycler) != null)
            {
                Recycler.access._mth2700(mRecycler).clear();
            }
            mViewInfoStore.clear();
...
_L5:
    ...
    protected void onMeasure(int i, int j)
    {
        if (mAdapterUpdateDuringMeasure)
        {
            ...
            if (State.access._mth2100(mState))
            {
                State.access._mth2202(mState, true);
            } else
            {
                mAdapterHelper.consumeUpdatesInOnePass();
                State.access._mth2202(mState, false);
    ...
    protected Parcelable onSaveInstanceState()
    {
        ...
        {
            SavedState.access._mth2000(savedstate, mPendingSavedState);
            return savedstate;
        }
        if (mLayout != null)
        {
...
label0:
            ...
            void start(RecyclerView recyclerview, LayoutManager layoutmanager)
            {
                ...
                {
                    State.access._mth5602(mRecyclerView.mState, mTargetPosition);
                    mRunning = true;
                    mPendingInitialRun = true;
            ...
            protected final void stop()
            {
                ...
                {
                    onStop();
                    State.access._mth5602(mRecyclerView.mState, -1);
                    mTargetView = null;
                    mTargetPosition = -1;
                    mPendingInitialRun = false;
                    mRunning = false;
