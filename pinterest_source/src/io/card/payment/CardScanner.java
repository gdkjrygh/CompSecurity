...
_L2:
    ...
    private boolean makePreviewGo(SurfaceHolder surfaceholder)
    {
        ...
        Log.d(TAG, (new StringBuilder("surfaceFrame: ")).append(String.valueOf(surfaceholder.getSurfaceFrame())).toString());
        mFirstPreviewFrame = true;
        if (useCamera)
        {
            try
...
_L6:
        ...
        {
            Log.d(TAG, (new StringBuilder("detected card: ")).append(detectioninfo.creditCard()).toString());
...
_L6:
        Log.d(TAG, (new StringBuilder("- parameters: ")).append(parameters).toString());
...
_L2:
        ...
        if (mCamera != null)
        {
            Log.v(TAG, (new StringBuilder("we already have a camera instance: ")).append(mCamera).toString());
        }
          goto _L9
          goto _L8
        size = null;
    ...
    {
        Log.v(TAG, (new StringBuilder("resumeScanning(")).append(surfaceholder).append(")").toString());
        if (mCamera == null)
        {
        ...
        {
            Log.v(TAG, (new StringBuilder("- mCamera:")).append(mCamera).toString());
            int i = mCamera.getParameters().getPreviewFormat();
            Log.v(TAG, (new StringBuilder("- preview format: ")).append(i).toString());
            i = ImageFormat.getBitsPerPixel(i) / 8;
            Log.v(TAG, (new StringBuilder("- bytes per pixel: ")).append(i).toString());
            i = i * 0x4b000 * 3;
            Log.v(TAG, (new StringBuilder("- buffer size: ")).append(i).toString());
    ...
    public boolean setFlashOn(boolean flag)
    {
        ...
        Log.d(TAG, (new StringBuilder("setFlashOn: ")).append(flag).toString());
        ...
        runtimeexception;
        Log.w(TAG, (new StringBuilder("Could not set flash mode: ")).append(runtimeexception).toString());
        return false;
    }
    ...
    void triggerAutoFocus(boolean flag)
    {
        ...
        if (flag)
        {
            ...
            {
                Log.w(TAG, (new StringBuilder("could not trigger auto focus: ")).append(runtimeexception).toString());
            }
            break MISSING_BLOCK_LABEL_80;
        }
    ...
    static 
    {
        ...
        Log.i("card.io", "card.io 5.1.2 10/28/2015 18:00:12 -0500");
        System.loadLibrary("cardioDecider");
        Log.d("card.io", "Loaded card.io decider library.");
        Log.d("card.io", (new StringBuilder("    nUseNeon(): ")).append(nUseNeon()).toString());
        Log.d("card.io", (new StringBuilder("    nUseTegra():")).append(nUseTegra()).toString());
        Log.d("card.io", (new StringBuilder("    nUseX86():  ")).append(nUseX86()).toString());
