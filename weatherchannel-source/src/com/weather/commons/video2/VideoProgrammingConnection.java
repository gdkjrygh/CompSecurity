...
_L1:
    ...
    private static class VideoFeedCacheLoader extends FileCacheLoader
    {
            ...
            throws DalException, ConfigException
        {
            ...
            String s1 = configurationManagers.getFlagshipConfig().videoProgrammingUrl;
            s1 = String.format(Locale.US, s1, new Object[] {
                s
            });
            LogUtil.d("VideoProgrammingConnection", LoggingMetaTags.TWC_VIDEOS, "hitting dsx programming api.  key=%s, url=%s", new Object[] {
                s, s1
            });
            return (new MetricsHttpGetRequest(new SimpleHttpGetRequest(), com.weather.util.metric.Metric.MetricTag.TIMER_VIDEO_PROGRAMMING_DATA_REQUEST)).fetch(s1, true).getBytes(Charset.forName("UTF-8"));
        }
    ...
    private static final String TAG = "VideoProgrammingConnection";
    private final FileCache cache;

    VideoProgrammingConnection(String s, ConfigurationManagers configurationmanagers, int i)
    {
        cache = FileCache.create(new VideoFeedCacheLoader(configurationmanagers), 1, i, s, com.weather.dal2.cache.FileCache.FileCacheDirectory.CACHE_DIR);
