...
_L5:
    ...
    static void validateManifest()
    {
        ...
        if (UAirship.shared().getAirshipConfigOptions().minSdkVersion < 16 || applicationinfo != null && applicationinfo.targetSdkVersion < 16 || android.os.Build.VERSION.SDK_INT < 16)
        {
            Object obj = (new StringBuilder()).append(s).append(".permission.C2D_MESSAGE").toString();
            if (isPermissionKnown(((String) (obj))))
            {
                ManifestUtils.checkRequiredPermission(((String) (obj)));
            } else
            {
                Logger.error((new StringBuilder()).append("Required permission ").append(((String) (obj))).append(" is unknown to PackageManager.").toString());
            }
        }
        if (ManifestUtils.getReceiverInfo(com/urbanairship/push/GCMPushReceiver) != null)
        {
            ...
            if (packagemanager.queryBroadcastReceivers(((Intent) (obj)), 0).isEmpty())
            {
                Logger.error((new StringBuilder()).append("AndroidManifest.xml's ").append(com/urbanairship/push/GCMPushReceiver.getCanonicalName()).append(" declaration missing required filter ").append(((Intent) (obj)).getAction()).append(". Your app may ").append("not be able to reset the GCM ID on app upgrade.").toString());
            }
            return;
        } else
        {
