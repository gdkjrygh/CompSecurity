...
//            LastLocationFinder, UALocationManager, LocationPreferences
...
public class LocationService extends Service
{
    ...
    public static final String ACTION_LOCATION_CHANGED = "com.urbanairship.location.ACTION_LOCATION_CHANGED";
    public static final String ACTION_RECORD_CURRENT_LOCATION = "com.urbanairship.location.RECORD_CURRENT_LOCATION";
    public static final String ACTION_SINGLE_LOCATION_CHANGED = "com.urbanairship.location.ACTION_SINGLE_LOCATION_CHANGED";
    ...
    {
        Intent intent = (new Intent(getApplicationContext(), com/urbanairship/location/LocationService)).setAction("com.urbanairship.location.ACTION_LOCATION_CHANGED").putExtra("com.urbanairship.location.REQUESTED_ACCURACY", criteria.getAccuracy());
    ...
    {
        Intent intent = (new Intent(getApplicationContext(), com/urbanairship/location/LocationService)).setAction("com.urbanairship.location.ACTION_SINGLE_LOCATION_CHANGED").putExtra("com.urbanairship.location.REQUESTED_ACCURACY", i);
        singleLocationChangedPendingIntent = PendingIntent.getService(context, 0, intent, 0x10000000);
    }
        ...
        AsyncTask asynctask = new AsyncTask() {
            ...
            protected transient Void doInBackground(Void avoid[])
            {
                ...
                if (avoid != null)
                {
                    ...
                    LocationEvent locationevent = new LocationEvent(avoid, com.urbanairship.analytics.LocationEvent.UpdateType.CONTINUOUS, criteria.getAccuracy(), UAirship.shared().getAirshipConfigOptions().locationOptions.updateIntervalMeters);
                    UAirship.shared().getAnalytics().addEvent(locationevent);
                    notifyNewLocation(avoid);
                } else
                {
    ...
    private void notifyNewLocation(Location location1)
    {
        Class class1 = UALocationManager.shared().getIntentReceiver();
        if (class1 != null)
        {
            location1 = (new Intent(context, class1)).setAction(".urbanairship.location.LOCATION_UPDATE").putExtra("com.urbanairship.location.LOCATION", location1);
            context.sendBroadcast(location1);
        }
    }

    private void onLocationChange(Intent intent)
    {
        Location location1 = (Location)intent.getExtras().get("location");
        if (location1 == null)
        {
            return;
        }
...
_L2:
        ...
        Logger.debug("Location Service stopping.");
        if (started)
        {
            teardownService();
            return;
...
_L3:
        ...
        Logger.debug("Location Service starting.");
        if (!started)
        {
            setupService();
        }
...
_L4:
        ...
        Logger.debug("Location Service recording current location.");
        if (!started)
        {
            setupService();
        }
