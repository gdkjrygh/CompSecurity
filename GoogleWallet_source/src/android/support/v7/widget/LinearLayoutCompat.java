...
public class LinearLayoutCompat extends ViewGroup
{
    ...
    private static int getChildrenSkipCount(View view, int i)
    {
        return 0;
    }

    private static int getLocationOffset(View view)
    {
        return 0;
    }

    private static int getNextLocationOffset(View view)
    {
        return 0;
    }
...
_L6:
    ...
    private void layoutHorizontal(int i, int j, int k, int l)
    {
        int ai[];
        int ai1[];
        int k1;
...
_L15:
        ...
        setChildFrame(view, getLocationOffset(view) + k, i, i4, j4);
...
_L16:
    ...
    private void layoutVertical(int i, int j, int k, int l)
    {
        int i1;
        int j1;
        int k1;
...
_L15:
    ...
    private void measureChildBeforeLayout(View view, int i, int j, int k, int l, int i1)
    {
...
_L3:
    ...
    private static int measureNullChild(int i)
    {
        return 0;
    }

    private void measureVertical(int i, int j)
    {
        float f;
        int k;
        int l;
        ...
        l = 1;
        f = 0.0F;
        j5 = getVirtualChildCount();
        k5 = android.view.View.MeasureSpec.getMode(i);
        l5 = android.view.View.MeasureSpec.getMode(j);
        k1 = 0;
        k2 = 0;
        i6 = mBaselineAlignedChildIndex;
...
label0:
        ...
        layoutparams = (LayoutParams)view.getLayoutParams();
        f += layoutparams.weight;
        if (l5 != 0x40000000 || layoutparams.height != 0 || layoutparams.weight <= 0.0F) goto _L5; else goto _L4
_L4:
        int l2;
        k2 = mTotalLength;
...
_L7:
        ...
        if (i2 < i6 && layoutparams.weight > 0.0F)
        {
...
_L5:
        ...
        measureChildBeforeLayout(view, i2, i, 0, j, l3);
        if (l2 != 0x80000000)
        {
        ...
        l2 = mTotalLength;
        mTotalLength = Math.max(l2, l2 + k4 + layoutparams.topMargin + layoutparams.bottomMargin + getNextLocationOffset(view));
        l3 = j2;
        l2 = k2;
        if (flag)
        {
...
_L6:
        ...
        k2 = l2;
          goto _L3
        if (mTotalLength > 0 && hasDividerBeforeChildAt(j5))
        {
            mTotalLength = mTotalLength + mDividerHeight;
        }
        if (flag && (l5 == 0x80000000 || l5 == 0))
        {
            mTotalLength = 0;
            i2 = 0;
            while (i2 < j5) 
            {
                ...
                } else
                {
                    LayoutParams layoutparams1 = (LayoutParams)view1.getLayoutParams();
                    int i3 = mTotalLength;
                    mTotalLength = Math.max(i3, i3 + j2 + layoutparams1.topMargin + layoutparams1.bottomMargin + getNextLocationOffset(view1));
                }
                i2++;
            }
        }
        mTotalLength = mTotalLength + (getPaddingTop() + getPaddingBottom());
        i6 = ViewCompat.resolveSizeAndState(Math.max(mTotalLength, getSuggestedMinimumHeight()), j, 0);
        i2 = (i6 & 0xffffff) - mTotalLength;
...
_L8:
        ...
        while (k2 < j5) 
        {
            ...
            if (view2.getVisibility() != 8)
            {
                ...
                if (f2 > 0.0F)
                {
                    l1 = (int)(((float)j2 * f2) / f);
                    f1 = f - f2;
                    i2 = j2 - l1;
                    j3 = getChildMeasureSpec(i, getPaddingLeft() + getPaddingRight() + layoutparams2.leftMargin + layoutparams2.rightMargin, layoutparams2.width);
                    if (layoutparams2.height != 0 || l5 != 0x40000000)
                    {
                        ...
                        view2.measure(j3, android.view.View.MeasureSpec.makeMeasureSpec(l1, 0x40000000));
                    } else
                    {
                        if (l1 <= 0)
                        {
                    ...
                    l1 = ViewUtils.combineMeasuredStates(i1, ViewCompat.getMeasuredState(view2) & 0xffffff00);
                }
                j2 = layoutparams2.leftMargin + layoutparams2.rightMargin;
                i4 = view2.getMeasuredWidth() + j2;
                j3 = Math.max(j1, i4);
                if (k5 != 0x40000000 && layoutparams2.width == -1)
                {
                    i1 = 1;
                } else
                {
...
_L11:
        ...
        setMeasuredDimension(ViewCompat.resolveSizeAndState(Math.max(l + (getPaddingLeft() + getPaddingRight()), getSuggestedMinimumWidth()), i, i2), i6);
        if (k1 != 0)
        {
            forceUniformWidth(j5, j);
        }
        return;
_L9:
        int j4 = Math.max(k, l1);
        k2 = l;
        k = j4;
        i2 = i1;
        l1 = j1;
        ...
        do
        {
            ...
            View view3 = getVirtualChildAt(k3);
