...
public final class ViewDragHelper
{
    public static abstract class Callback
    {
        ...
        public static int getViewVerticalDragRange(View view)
        {
            return 0;
        }
    ...
    private boolean checkTouchSlop(int i, int j)
    {
...
_L5:
    ...
    private int computeSettleDuration(View view, int i, int j, int k, int l)
    {
        ...
        callback = mCallback;
        j = computeAxisDuration(j, l, Callback.getViewVerticalDragRange(view));
    ...
    private static ViewDragHelper create(ViewGroup viewgroup, Callback callback)
    {
    ...
    private boolean isCapturedViewUnder(int i, int j)
    {
    ...
    public final View findTopChildUnder(int i, int j)
    {
        ...
        {
            Object obj = mParentView;
            Callback callback = mCallback;
            obj = ((ViewGroup) (obj)).getChildAt(Callback.getOrderedChildIndex(k));
            if (i >= ((View) (obj)).getLeft() && i < ((View) (obj)).getRight() && j >= ((View) (obj)).getTop() && j < ((View) (obj)).getBottom())
            {
                return ((View) (obj));
            }
        }
...
_L12:
        j = MotionEventCompat.getPointerId(motionevent, i);
        if (j == mActivePointerId) goto _L15; else goto _L14
_L14:
        f1 = MotionEventCompat.getX(motionevent, i);
        f3 = MotionEventCompat.getY(motionevent, i);
        if (findTopChildUnder((int)f1, (int)f3) != mCapturedView || !tryCaptureViewForDrag(mCapturedView, j)) goto _L15; else goto _L16
_L16:
        j = mActivePointerId;
_L13:
        if (j == -1)
...
_L5:
        ...
        if (mDragState == 0)
        {
            ...
            {
                mCallback.onEdgeTouched(mTrackingEdges & i, k);
            }
        } else
        if (mDragState == 2)
        {
...
_L4:
        ...
        do
        {
            ...
            if (flag)
            {
                int j1 = view.getLeft();
                int k1 = (int)f6;
                k1 = mCallback.clampViewPositionHorizontal(view, j1 + k1, (int)f6);
                int l1 = view.getTop();
                int i2 = (int)f7;
                i2 = mCallback.clampViewPositionVertical(view, l1 + i2, (int)f7);
                int j2 = mCallback.getViewHorizontalDragRange(view);
                Callback callback = mCallback;
                int k2 = Callback.getViewVerticalDragRange(view);
