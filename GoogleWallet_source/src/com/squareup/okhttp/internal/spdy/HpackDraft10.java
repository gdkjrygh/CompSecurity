...
label0:
        ...
        private ByteString getName(int i)
        {
            ...
            {
                return dynamicTable[dynamicTableIndex(i - HpackDraft10.STATIC_HEADER_TABLE.length)].name;
            }
        }
            ...
            throws IOException
        {
            if (isStaticHeader(i))
            {
                Header header = HpackDraft10.STATIC_HEADER_TABLE[i];
                headerList.add(header);
                return;
            }
            int j = dynamicTableIndex(i - HpackDraft10.STATIC_HEADER_TABLE.length);
        ...
        private void readLiteralHeaderWithoutIndexingNewName()
            throws IOException
        {
            ByteString bytestring = HpackDraft10.checkLowercase(readByteString());
            ByteString bytestring1 = readByteString();
            headerList.add(new Header(bytestring, bytestring1));
        }
    ...
    static final class Writer
    {
        ...
        private void writeByteString(ByteString bytestring)
            throws IOException
        {
        ...
        private void writeInt(int i, int j, int k)
            throws IOException
        {
            if (i < j)
            {
            ...
            throws IOException
        {
            ...
            while (i < j) 
            {
                ...
                Integer integer = (Integer)HpackDraft10.NAME_TO_FIRST_INDEX.get(bytestring);
                if (integer != null)
                {
        ...
        throws IOException
    {
        ...
        for (int j = bytestring.size(); i < j; i++)
        {
            ...
            {
                throw new IOException((new StringBuilder("PROTOCOL_ERROR response malformed: mixed case name: ")).append(bytestring.utf8()).toString());
            }
        }
