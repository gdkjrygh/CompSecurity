...
public final class HttpEngine
{
        ...
        implements com.squareup.okhttp.Interceptor.Chain
    {
        ...
        public final Response proceed(Request request1)
            throws IOException
        {
            calls = calls + 1;
            if (index > 0)
    ...
    private boolean isRecoverable(IOException ioexception, Connection connection1)
    {
        if (client.getRetryOnConnectionFailure() && !(ioexception instanceof SSLPeerUnverifiedException) && (!(ioexception instanceof SSLHandshakeException) || !(ioexception.getCause() instanceof CertificateException)) && !(ioexception instanceof ProtocolException))
        {
            if (ioexception instanceof SocketTimeoutException)
            {
                System.err.println((new StringBuilder("Attempting recover after: ")).append(ioexception.getMessage()).toString());
                return true;
            }
    ...
    private HttpEngine recover(IOException ioexception, Sink sink)
    {
        ...
        if (routeSelector == null && connection == null || routeSelector != null && !routeSelector.hasNext() || !isRecoverable(ioexception, connection) || !flag)
        {
            return null;
        } else
        {
