...
public final class HighLevelEncoder
{
    ...
    private static List simplifyStates(Iterable iterable)
    {
...
label1:
    ...
    private List updateStateListForChar(Iterable iterable, int i)
    {
        LinkedList linkedlist = new LinkedList();
    ...
    private static List updateStateListForPair(Iterable iterable, int i, int j)
    {
        LinkedList linkedlist = new LinkedList();
    ...
    public final BitArray encode()
    {
        List list;
        int i;
        list = Collections.singletonList(State.INITIAL_STATE);
        i = 0;
_L6:
        byte byte0;
        if (i >= text.length)
...
_L7:
        if (byte0 > 0)
        {
            list = updateStateListForPair(list, i, byte0);
            i++;
        } else
        {
            list = updateStateListForChar(list, i);
        }
        i++;
...
_L5:
        ...
        } else
        {
          ...
          goto _L7
        return ((State)Collections.min(list, new Comparator() {
    ...
    static 
    {
        Object obj = (int[][])Array.newInstance(Integer.TYPE, new int[] {
            5, 256
        });
        CHAR_MAP = ((int [][]) (obj));
        obj[0][32] = 1;
