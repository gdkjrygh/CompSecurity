...
final class MatrixUtil
{
    ...
    private static int calculateBCHCode(int i, int j)
    {
        int k = findMSBSet(j);
    ...
    private static void clearMatrix(ByteMatrix bytematrix)
    {
        bytematrix.clear((byte)-1);
    }

    private static void embedBasicPatterns(Version version, ByteMatrix bytematrix)
        throws WriterException
    {
    ...
    private static void embedDataBits(BitArray bitarray, int i, ByteMatrix bytematrix)
        throws WriterException
    {
        int l1 = 0;
        int l = -1;
        ...
        while (k > 0) 
        {
            ...
            while (j1 >= 0 && j1 < bytematrix.getHeight()) 
            {
                ...
                while (k < 2) 
                {
                    l1 = i1 - k;
                    k1 = j;
                    if (isEmpty(bytematrix.get(l1, j1)))
                    {
                        boolean flag;
                        boolean flag1;
                        ...
                        {
                            flag1 = flag;
                            if (MaskUtil.getDataMaskBit(i, l1, j1))
                            {
                                if (!flag)
                                {
                                    flag1 = true;
        ...
        {
            throw new WriterException((new StringBuilder("Not all bits consumed: ")).append(l1).append('/').append(bitarray.getSize()).toString());
        } else
        {
            return;
        }
    ...
    private static void embedTypeInfo(ErrorCorrectionLevel errorcorrectionlevel, int i, ByteMatrix bytematrix)
        throws WriterException
    {
    ...
    private static int findMSBSet(int i)
    {
        boolean flag = false;
        int j = i;
    ...
    private static void makeTypeInfoBits(ErrorCorrectionLevel errorcorrectionlevel, int i, BitArray bitarray)
        throws WriterException
    {
    ...
    private static void makeVersionInfoBits(Version version, BitArray bitarray)
        throws WriterException
    {
    ...
    private static void maybeEmbedVersionInfo(Version version, ByteMatrix bytematrix)
        throws WriterException
    {
