...
_L5:
        ...
        j = ((flag) ? 1 : 0);
        if (bitsource.available() == 0) goto _L3; else goto _L6
_L6:
        throw FormatException.getFormatInstance();
...
_L7:
    ...
    private static void decodeBase256Segment(BitSource bitsource, StringBuilder stringbuilder, Collection collection)
        throws FormatException
    {
        int j = bitsource.getByteOffset() + 1;
        int k = bitsource.readBits(8);
        int i = j + 1;
        k = unrandomize255State(k, j);
        if (k == 0)
        {
        ...
        {
            int i1 = bitsource.readBits(8);
            j = i + 1;
            k = (k - 249) * 250 + unrandomize255State(i1, i);
            i = j;
            j = k;
        }
        if (j < 0)
        ...
        for (int l = 0; l < j;)
        {
            ...
            abyte0[l] = (byte)unrandomize255State(bitsource.readBits(8), i);
            l++;
            i++;
        }
        ...
        {
            throw new IllegalStateException((new StringBuilder("Platform does not support required encoding: ")).append(bitsource).toString());
        }
    }
