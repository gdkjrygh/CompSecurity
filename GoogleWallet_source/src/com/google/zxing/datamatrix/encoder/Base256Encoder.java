    ...
    implements Encoder
{
    ...
    private static int getEncodingMode()
    {
        return 5;
    }
    ...
    public final void encode(EncoderContext encodercontext)
    {
        ...
        if (encodercontext.hasMoreCharacters() || j != 0)
        {
            ...
            {
                throw new IllegalStateException((new StringBuilder("Message length not in valid ranges: ")).append(k).toString());
            }
        }
        j = 0;
        for (k = stringbuilder.length(); j < k; j++)
        {
