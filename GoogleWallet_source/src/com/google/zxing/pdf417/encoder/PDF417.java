...
public final class PDF417
{
        ...
        throws WriterException
    {
        ...
        int ai[] = null;
label0:
        for (int k = minCols; k <= maxCols;)
        {
            float f1;
            int ai1[];
label1:
            {
                int l = calculateNumberOfRows(i, j, k);
                if (l < minRows)
                {
                    break label0;
                }
                ...
                float f2 = ((float)(k * 17 + 69) * 0.357F) / ((float)l * 2.0F);
                if (ai != null)
                {
                    ai1 = ai;
                    f1 = f;
                    if (Math.abs(f2 - 3F) > Math.abs(f - 3F))
                    {
                        break label1;
                    }
                }
    ...
    public final void generateBarcodeLogic(String s, int i)
        throws WriterException
    {
        int j = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(i);
        String s1 = PDF417HighLevelEncoder.encodeHighLevel(s, compaction);
        int k1 = s1.length();
        int ai[] = determineDimensions(k1, j);
        int l = ai[0];
        int i1 = ai[1];
        int j1 = getNumberOfPadCodewords(k1, j, l, i1);
        if (k1 + j + 1 > 929)
        {
        ...
        barcodeMatrix = new BarcodeMatrix(i1, l);
        encodeLowLevel(s, l, i1, i, barcodeMatrix);
    }
    ...
    public final void setCompaction(Compaction compaction1)
    {
