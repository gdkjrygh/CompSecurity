...
public final class CodaBarWriter extends OneDimensionalCodeWriter
{
    ...
    public final boolean[] encode(String s)
    {
        ...
        if (!flag && !flag1)
        {
            throw new IllegalArgumentException((new StringBuilder("Codabar should start/end with ")).append(Arrays.toString(START_END_CHARS)).append(", or start/end with ").append(Arrays.toString(ALT_START_END_CHARS)).toString());
        }
        int i = 20;
        l = 1;
        while (l < s.length() - 1) 
    ...
    '/', ':', '+', '.'
                ...
                {
                    throw new IllegalArgumentException((new StringBuilder("Cannot encode : '")).append(s.charAt(l)).append('\'').toString());
                }
            }
            l++;
        }
