...
static final class expirationQueue extends ReentrantLock
{
    ...
    private eEntry copyEntry(eEntry eentry, eEntry eentry1)
    {
    ...
    private void enqueueNotification(eEntry eentry, eEntry eentry1)
    {
        enqueueNotification(eentry.getKey(), eentry.getHash(), eentry.getValueReference().get(), eentry1);
    }

    private void enqueueNotification(Object obj, int i, Object obj1, erence.get get1)
    {
...
_L5:
        ...
        if (eentry == null) goto _L1; else goto _L3
_L3:
        MapMakerInternalMap mapmakerinternalmap = map;
        if (!MapMakerInternalMap.isExpired(eentry, l)) goto _L1; else goto _L4
_L4:
    ...
    private eEntry getEntry(Object obj, int i)
    {
        eEntry eentry;
        if (count == 0)
        {
...
_L3:
    ...
    private static boolean isCollected(erence erence)
    {
    ...
    private eEntry newEntry(Object obj, int i, eEntry eentry)
    {
    ...
    private void recordExpirationTime(eEntry eentry, long l)
    {
        eentry.setExpirationTime(map.ticker.read() + l);
    }

    private void recordLockedRead(eEntry eentry)
    {
        evictionQueue.add(eentry);
    ...
    private void recordRead(eEntry eentry)
    {
        if (map.expiresAfterAccess())
        {
            recordExpirationTime(eentry, map.expireAfterAccessNanos);
    ...
    private void recordWrite(eEntry eentry)
    {
        drainRecencyQueue();
        evictionQueue.add(eentry);
        if (map.expires())
    ...
    private void removeCollectedEntry(eEntry eentry)
    {
    ...
    private boolean removeEntry(eEntry eentry, int i, eEntry eentry1)
    {
        int j = count;
        AtomicReferenceArray atomicreferencearray = table;
    ...
    private eEntry removeFromChain(eEntry eentry, eEntry eentry1)
    {
    ...
    private void setValue(eEntry eentry, Object obj)
    {
...
_L3:
    ...
    final Object getLiveValue(eEntry eentry)
    {
        Object obj;
        if (eentry.getKey() == null)
        {
...
_L3:
    ...
    final boolean reclaimKey(eEntry eentry, int i)
    {
        lock();
        eEntry eentry2;
        AtomicReferenceArray atomicreferencearray;
...
_L1:
    ...
    final boolean reclaimValue(Object obj, int i, erence erence)
    {
        boolean flag;
        flag = false;
        lock();
