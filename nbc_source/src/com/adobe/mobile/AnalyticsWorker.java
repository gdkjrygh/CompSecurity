...
_L1:
    ...
    private static String getBaseURL()
    {
        if (analyticsGetBaseURL_pred)
        {
            ...
            baseURL = stringbuilder.append(s).append(MobileConfig.getInstance().getTrackingServer()).append("/b/ss/").append(StaticMethods.URLEncode(MobileConfig.getInstance().getReportSuiteIds())).append("/0/JAVA-").append("4.3.0-AN").append("/s").toString();
...
_L3:
    ...
    protected void kickWithReferrerData(Map map)
    {
        ...
        if (hit != null)
        {
            hit.urlFragment = (new StringBuilder()).append(hit.urlFragment).append(StaticMethods.serializeToQueryString(map)).toString();
