...
public abstract class EventsFilesManager
{
    ...
    EventsFilesManager(Context context1, EventTransform eventtransform, CurrentTimeProvider currenttimeprovider, EventsStorage eventsstorage)
        throws IOException
    {
        this(context1, eventtransform, currenttimeprovider, eventsstorage, 100);
    }
    ...
    public EventsFilesManager(Context context1, EventTransform eventtransform, CurrentTimeProvider currenttimeprovider, EventsStorage eventsstorage, int i)
        throws IOException
    {
        rollOverListeners = new CopyOnWriteArrayList();
    ...
    public void deleteOldestInRollOverIfOverMax()
    {
        ...
        if (((List) (obj1)).size() <= maxFilesToKeep)
        {
            return;
        }
        int i = ((List) (obj1)).size() - maxFilesToKeep;
        CommonUtils.logControlled(context, String.format(Locale.US, "Found %d files in  roll over directory, this is greater than %d, deleting %d oldest files", new Object[] {
            Integer.valueOf(((List) (obj1)).size()), Integer.valueOf(maxFilesToKeep), Integer.valueOf(i)
        }));
