...
public final class PersonEntity_AgeRangeEntityCreator
    implements android.os.Parcelable.Creator
{

    public PersonEntity_AgeRangeEntityCreator()
    {
    }

    static void writeToParcel$665c4158(PersonEntity.AgeRangeEntity agerangeentity, Parcel parcel)
    {
        int i = SafeParcelWriter.beginVariableData(parcel, 20293);
        Set set = agerangeentity.mIndicatorSet;
        if (set.contains(Integer.valueOf(1)))
        {
            SafeParcelWriter.writeInt(parcel, 1, agerangeentity.mVersionCode);
        }
        if (set.contains(Integer.valueOf(2)))
        {
            SafeParcelWriter.writeInt(parcel, 2, agerangeentity.mMax);
        }
        if (set.contains(Integer.valueOf(3)))
        {
            SafeParcelWriter.writeInt(parcel, 3, agerangeentity.mMin);
        }
