...
_L5:
    ...
    private boolean overScrollByCompat$30fc967d$69c647f9(int i, int j, int k, int l, int i1)
    {
    ...
    private boolean scrollAndFocus(int i, int j, int k)
    {
        ...
        obj = (View)arraylist.get(i1);
        j1 = ((View) (obj)).getTop();
        j2 = ((View) (obj)).getBottom();
        if (j >= j2 || j1 >= k)
        {
...
label0:
            {
                if (flag1 && j1 < view.getTop() || !flag1 && j2 > view.getBottom())
                {
                    j1 = 1;
                } else
                {
...
_L3:
    ...
    private void smoothScrollBy(int i, int j)
    {
        if (getChildCount() == 0)
        {
            return;
        ...
        if (AnimationUtils.currentAnimationTimeMillis() - mLastScroll > 250L)
        {
            ...
            i = getPaddingTop();
            k = Math.max(0, getChildAt(0).getHeight() - (k - l - i));
            i = getScrollY();
            Math.max(0, Math.min(i + j, k));
            getScrollX();
            throw new NullPointerException();
        } else
        {
...
_L10:
        ...
        if (j != 0)
        {
            ...
            if (j > 0)
            {
                ...
                if (mTempRect.top + k > keyevent.getBottom())
                {
                    mTempRect.top = keyevent.getBottom() - k;
                }
            }
        } else
        {
        ...
        mTempRect.bottom = k + mTempRect.top;
    ...
    protected final void measureChild(View view, int i, int j)
    {
...
_L4:
        ...
        if (l != -1)
        {
            int j = MotionEventCompat.findPointerIndex(motionevent, l);
            if (j == -1)
            {
                Log.e("NestedScrollView", (new StringBuilder("Invalid pointerId=")).append(l).append(" in onInterceptTouchEvent").toString());
            } else
            {
                j = (int)MotionEventCompat.getY(motionevent, j);
                if (Math.abs(j - mLastMotionY) > 0 && (getNestedScrollAxes() & 2) == 0)
                {
...
_L2:
        ...
        if (getChildCount() > 0)
        {
            int k = getScrollY();
            View view = getChildAt(0);
            if (i1 >= view.getTop() - k && i1 < view.getBottom() - k && j1 >= view.getLeft() && j1 < view.getRight())
            {
                flag = true;
            } else
            {
...
_L5:
    ...
    public final boolean onNestedPreFling(View view, float f, float f1)
    {
        return false;
    }
    ...
    public final void onNestedScrollAccepted(View view, View view1, int i)
    {
...
_L4:
        ...
        if (overScrollByCompat$30fc967d$69c647f9(0, i, 0, getScrollY(), j1) && !hasNestedScrollingParent())
        {
            mVelocityTracker.clear();
        }
        l1 = getScrollY() - k1;
...
_L3:
        if (mIsBeingDragged)
        {
            ...
            motionevent.computeCurrentVelocity(1000, 0.0F);
