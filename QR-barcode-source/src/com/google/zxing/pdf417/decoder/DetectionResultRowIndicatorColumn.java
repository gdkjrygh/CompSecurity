...
_L2:
    ...
    int adjustCompleteIndicatorColumnRowNumbers(BarcodeMetadata barcodemetadata)
    {
        ...
        k2 = imageRowToCodewordIndex((int)resultpoint.getY());
        k3 = imageRowToCodewordIndex((int)((ResultPoint) (obj)).getY());
        f = (float)(k3 - k2) / (float)barcodemetadata.getRowCount();
        j = -1;
        k = 0;
        i = 1;
        while (k2 < k3) 
        {
            ...
            {
                Codeword codeword = acodeword[k2];
                int l2 = codeword.getRowNumber() - j;
                if (l2 == 0)
                {
                    l2 = k + 1;
                    k = i;
