    ...
    public void onItemClick(AdapterView adapterview, View view, int i, long l)
    {
        if (i >= 1)
        {
            if (i-- < items.size())
            {
                ...
                if (LocaleManager.isBookSearchUrl(activity.getISBN()) && adapterview.length() > 0)
                {
                    String s = activity.getISBN();
                    s = s.substring(s.indexOf('=') + 1);
                    adapterview = new Intent("android.intent.action.VIEW", Uri.parse((new StringBuilder("http://books.google.")).append(LocaleManager.getBookSearchCountryTLD(activity)).append("/books?id=").append(s).append("&pg=").append(adapterview).append("&vq=").append(view).toString()));
