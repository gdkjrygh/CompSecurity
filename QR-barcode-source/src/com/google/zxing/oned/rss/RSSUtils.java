// Decompiled by Jad v1.5.8e. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.geocities.com/kpdus/jad.html
// Decompiler options: braces fieldsfirst space lnc 

package com.google.zxing.oned.rss;


public final class RSSUtils
{

    private RSSUtils()
    {
    }

    private static int combins(int i, int j)
    {
        int j1 = 1;
        int k;
        int l;
        int i1;
        if (i - j > j)
        {
            l = i - j;
            k = j;
        } else
        {
            l = j;
            k = i - j;
        }
        j = 1;
        i1 = i;
        for (i = j1; i1 > l; i = j1)
        {
            int k1 = j * i1;
            j1 = i;
            j = k1;
            if (i <= k)
            {
                j = k1 / i;
                j1 = i + 1;
            }
            i1--;
        }

        for (; i <= k; i++)
        {
            j /= i;
        }

        return j;
    }

    static int[] elements(int ai[], int i, int j)
    {
        int ai1[] = new int[ai.length + 2];
        int k1 = j << 1;
        ai1[0] = 1;
        j = 10;
        int i1 = 1;
        for (int k = 1; k < k1 - 2;)
        {
            ai1[k] = ai[k - 1] - ai1[k - 1];
            ai1[k + 1] = ai[k] - ai1[k];
            int j1 = i1 + (ai1[k] + ai1[k + 1]);
            i1 = j;
            if (ai1[k] < j)
            {
                i1 = ai1[k];
            }
            k += 2;
            j = i1;
            i1 = j1;
        }

        ai1[k1 - 1] = i - i1;
        if (ai1[k1 - 1] < j)
        {
            i = ai1[k1 - 1];
        } else
        {
            i = j;
        }
        if (i > 1)
        {
            for (j = 0; j < k1; j += 2)
            {
                ai1[j] = ai1[j] + (i - 1);
                int l = j + 1;
                ai1[l] = ai1[l] - (i - 1);
            }

        }
        return ai1;
    }

    public static int getRSSvalue(int ai[], int i, boolean flag)
    {
        int l2 = ai.length;
        int l = ai.length;
        int k = 0;
        int j;
        int j1;
        for (j = 0; k < l; j = j1 + j)
        {
            j1 = ai[k];
            k++;
        }

        int i2 = 0;
        k = 0;
        l = 0;
        int l1 = j;
        j = k;
        k = l;
        do
        {
            if (i2 >= l2 - 1)
            {
                break;
            }
            int i1 = k | 1 << i2;
            int j2 = 1;
            k = j;
            j = i1;
            while (j2 < ai[i2]) 
            {
                int k1 = combins(l1 - j2 - 1, l2 - i2 - 2);
                i1 = k1;
                if (flag)
                {
                    i1 = k1;
                    if (j == 0)
                    {
                        i1 = k1;
                        if (l1 - j2 - (l2 - i2 - 1) >= l2 - i2 - 1)
                        {
                            i1 = k1 - combins(l1 - j2 - (l2 - i2), l2 - i2 - 2);
                        }
                    }
                }
                if (l2 - i2 - 1 > 1)
                {
                    k1 = l1 - j2 - (l2 - i2 - 2);
                    int k2 = 0;
                    for (; k1 > i; k1--)
                    {
                        k2 += combins(l1 - j2 - k1 - 1, l2 - i2 - 3);
                    }

                    k1 = i1 - (l2 - 1 - i2) * k2;
                } else
                {
                    k1 = i1;
                    if (l1 - j2 > i)
                    {
                        k1 = i1 - 1;
                    }
                }
                k += k1;
                j2++;
                j &= ~(1 << i2);
            }
            i2++;
            l1 -= j2;
            i1 = k;
            k = j;
            j = i1;
        } while (true);
        return j;
    }

    static int[] getRSSwidths(int i, int j, int k, int l, boolean flag)
    {
        int ai[];
        int l1;
        int i2;
        ai = new int[k];
        boolean flag1 = false;
        l1 = 0;
        i2 = j;
        j = ((flag1) ? 1 : 0);
_L9:
        if (l1 >= k - 1) goto _L2; else goto _L1
_L1:
        int j2;
        int k2;
        j |= 1 << l1;
        j2 = 1;
        k2 = i;
        i = j;
_L7:
        int i1 = combins(i2 - j2 - 1, k - l1 - 2);
        j = i1;
        if (flag)
        {
            j = i1;
            if (i == 0)
            {
                j = i1;
                if (i2 - j2 - (k - l1 - 1) >= k - l1 - 1)
                {
                    j = i1 - combins(i2 - j2 - (k - l1), k - l1 - 2);
                }
            }
        }
        if (k - l1 - 1 <= 1) goto _L4; else goto _L3
_L3:
        int k1;
        int l2 = 0;
        for (int j1 = i2 - j2 - (k - l1 - 2); j1 > l; j1--)
        {
            l2 += combins(i2 - j2 - j1 - 1, k - l1 - 3);
        }

        k1 = j - (k - 1 - l1) * l2;
_L6:
        k2 -= k1;
        if (k2 >= 0)
        {
            break; /* Loop/switch isn't completed */
        }
        k1 = k2 + k1;
        i2 -= j2;
        ai[l1] = j2;
        l1++;
        j = i;
        i = k1;
        continue; /* Loop/switch isn't completed */
_L4:
        k1 = j;
        if (i2 - j2 > l)
        {
            k1 = j - 1;
        }
        if (true) goto _L6; else goto _L5
_L5:
        j2++;
        i &= ~(1 << l1);
        if (true) goto _L7; else goto _L2
_L2:
        ai[l1] = i2;
        return ai;
        if (true) goto _L9; else goto _L8
_L8:
    }
}
