// Decompiled by Jad v1.5.8e. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.geocities.com/kpdus/jad.html
// Decompiler options: braces fieldsfirst space lnc 

package com.google.android.gms.plus;

import com.google.android.gms.common.api.Status;

// Referenced classes of package com.google.android.gms.plus:
//            PlusClient

class ale
    implements com.google.android.gms.common.api.ation.b
{

    final MomentsLoadedListener ale;
    final PlusClient alf;

    public void a(omentsResult omentsresult)
    {
        ale.onMomentsLoaded(omentsresult.getStatus().gt(), omentsresult.getMomentBuffer(), omentsresult.getNextPageToken(), omentsresult.getUpdated());
    }

    public void b(Object obj)
    {
        a((omentsResult)obj);
    }

    MomentsLoadedListener(PlusClient plusclient, MomentsLoadedListener momentsloadedlistener)
    {
        alf = plusclient;
        ale = momentsloadedlistener;
        super();
    }
}
