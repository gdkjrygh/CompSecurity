...
public class HitBuilders
{
    ...
    protected static class HitBuilder
    {
        ...
        public HitBuilder addImpression(Product product, String s)
        {
            ...
            if (!AK.containsKey(s1))
            {
                AK.put(s1, new ArrayList());
            }
            ((List)AK.get(s1)).add(product);
            return this;
        }
        ...
        public Map build()
        {
            HashMap hashmap = new HashMap(AI);
            if (AJ != null)
            {
                hashmap.putAll(AJ.build());
            }
            Iterator iterator = AL.iterator();
            for (int i = 1; iterator.hasNext(); i++)
            {
                hashmap.putAll(((Promotion)iterator.next()).aq(n.A(i)));
            }

            iterator = AM.iterator();
            for (int j = 1; iterator.hasNext(); j++)
            {
                hashmap.putAll(((Product)iterator.next()).aq(n.z(j)));
            }

            iterator = AK.entrySet().iterator();
            for (int k = 1; iterator.hasNext(); k++)
            {
                ...
                {
                    hashmap.putAll(((Product)((Iterator) (obj)).next()).aq((new StringBuilder()).append(s).append(n.B(l)).toString()));
                }
