...
_L6:
    ...
    {
        int j = getCommonPrefix(new String[] {
...
_L5:
    ...
    public static double getJaroWinklerDistance(CharSequence charsequence, CharSequence charsequence1)
    {
        ...
        } else
        {
            ...
            return (double)Math.round((0.10000000000000001D * (double)commonPrefixLength(charsequence, charsequence1) * (1.0D - d) + d) * 100D) / 100D;
        }
    }
...
_L13:
    ...
    private static double score(CharSequence charsequence, CharSequence charsequence1)
    {
        ...
        for (s1 = getSetOfMatchingCharacterWithin(charsequence, charsequence1, i); s.length() == 0 || s1.length() == 0 || s.length() != s1.length();)
        {
            return 0.0D;
        }

        i = transpositions(s, s1);
        return ((double)s.length() / (double)charsequence1.length() + (double)s1.length() / (double)charsequence.length() + (double)(s.length() - i) / (double)s.length()) / 3D;
    }
