...
public class Utils
{
    ...
    {
        String s = "iframe, img { width: 100%; }";
        if (isDarkTheme(context))
        {
            webview.setBackgroundColor(0xff000000);
            s = (new StringBuilder()).append("iframe, img { width: 100%; }").append("body { background-color: #000000; color: #F6F6F6; } a { color: #0099FF; }").toString();
        }
        article = String.format("<html><head><meta charset=\"utf-8\"><link rel=\"stylesheet\" type=\"text/css\" href=\"http://cdn.uservoice.com/stylesheets/vendor/typeset.css\"/><style>%s</style></head><body class=\"typeset\" style=\"font-family: sans-serif; margin: 1em\"><h3>%s</h3>%s</body></html>", new Object[] {
    ...
    public static void showModel(FragmentActivity fragmentactivity, BaseModel basemodel)
    {
        if (basemodel instanceof Article)
        {
            (new ArticleDialogFragment((Article)basemodel)).show(fragmentactivity.getSupportFragmentManager(), "ArticleDialogFragment");
        } else
        {
            if (basemodel instanceof Suggestion)
            {
                (new SuggestionDialogFragment((Suggestion)basemodel)).show(fragmentactivity.getSupportFragmentManager(), "SuggestionDialogFragment");
                return;
            }
            if (basemodel instanceof Topic)
            {
                Session.getInstance().setTopic((Topic)basemodel);
                fragmentactivity.startActivity(new Intent(fragmentactivity, com/uservoice/uservoicesdk/activity/TopicActivity));
                return;
            }
        }
    }
