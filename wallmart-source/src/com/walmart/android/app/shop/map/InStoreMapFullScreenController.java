...
//            InStoreMapController, AisleAdapter, InStoreMapMarkerView, SourceDataMapper, 
//            MappedData, InStoreMapView, AisleMapTransform, AisleSection, 
//            MappedSections

public class InStoreMapFullScreenController extends InStoreMapController
{
    public static interface MarkerCallbacks
        extends InStoreMapController.LoadingCallbacks
    {

        public abstract void onMapTap();

        public abstract void onMarkerTap(List list);
    }

    private static class ZoomToMarker
        implements InStoreMapView.Zoom
    {
        ...
        public ZoomToMarker(AisleMapTransform aislemaptransform, AisleSection aislesection, Rect rect, boolean flag)
        {
    ...
    private static final String TAG = com/walmart/android/app/shop/map/InStoreMapFullScreenController.getSimpleName();
    private boolean mFirstZoomDone;
    private final MarkerCallbacks mMarkerCallbacks;
    private Rect mVisibleRect;

    public InStoreMapFullScreenController(Context context, SourceDataMapper sourcedatamapper, MarkerCallbacks markercallbacks)
    {
        super(context, sourcedatamapper, markercallbacks);
        mMarkerCallbacks = markercallbacks;
        mStoreMapAdapter.setPinResource(0x7f0201b4);
    }

    protected InStoreMapMarkerView createMapView(Context context, ViewGroup viewgroup)
    {
        return (InStoreMapMarkerView)ViewUtil.inflate(context, 0x7f04006e, viewgroup, false);
    }

    public MappedData currentPosition()
    {
        return mSourceDataMapper.getSelected();
    }

    protected void destroyMapView(InStoreMapMarkerView instoremapmarkerview)
    {
        super.destroyMapView(instoremapmarkerview);
        instoremapmarkerview.setMarkerEventListener(null);
        mStoreMapAdapter.setSelectedData(null);
    }

    protected void initMapView(InStoreMapMarkerView instoremapmarkerview)
    {
        super.initMapView(instoremapmarkerview);
        instoremapmarkerview.setBlockTouch(false);
        instoremapmarkerview.setMapTapListener(new InStoreMapView.MapTapListener() {

            final InStoreMapFullScreenController this$0;

            public void onMapTapped()
            {
                WLog.d(InStoreMapFullScreenController.TAG, "onMapTapped");
                mMarkerCallbacks.onMapTap();
            }

            public void onMarkerTapped(Marker marker)
            {
                WLog.i(InStoreMapFullScreenController.TAG, (new StringBuilder()).append("Marker selected position:").append(Arrays.toString(marker.getPosition())).append(", anchor:").append(Arrays.toString(marker.getAnchor())).toString());
                if (marker instanceof AisleAdapter.AisleMarker)
                {
                    mMarkerCallbacks.onMarkerTap(mSourceDataMapper.getMappedSections().getData(((AisleAdapter.AisleMarker)marker).getAisleSection()));
                }
            }
    ...
    public void moveTo(MappedData mappeddata)
    {
        if (mappeddata != null && mMapView != null)
        {
            mappeddata = mSourceDataMapper.select(mappeddata);
            if (mappeddata != null)
            {
                InStoreMapMarkerView instoremapmarkerview = mMapView;
                AisleMapTransform aislemaptransform = mAisleMapTransform;
                ...
                instoremapmarkerview.zoom(new ZoomToMarker(aislemaptransform, mappeddata, rect, flag));
                mFirstZoomDone = true;
                mStoreMapAdapter.setSelectedData(mappeddata);
            }
        }
    }
    ...
    {
        AisleSection aislesection = mStoreMapAdapter.getSelected();
        if (!mFirstZoomDone && aislesection != null && mMapView != null)
        {
            mMapView.zoom(new ZoomToMarker(mAisleMapTransform, aislesection, mVisibleRect, true));
            mFirstZoomDone = true;
        }
    }
