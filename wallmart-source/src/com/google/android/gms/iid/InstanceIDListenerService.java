...
public class InstanceIDListenerService extends Service
{
    ...
    public InstanceIDListenerService()
    {
        zzaAV = new MessengerCompat(new Handler(Looper.getMainLooper()) {
            ...
            {
                zzaBb = InstanceIDListenerService.this;
                super(looper);
            }
        });
    ...
    private void zza(Message message, int i)
    {
        ...
        {
            Log.w("InstanceID", (new StringBuilder()).append("Message from unexpected caller ").append(i).append(" mine=").append(zzc.zzaBg).append(" appid=").append(zzc.zzaBh).toString());
            return;
        } else
        {
...
_L1:
        ...
        ((InstanceID) (obj)).zzvB().zzr(intent);
_L4:
        return;
_L2:
        if (Log.isLoggable("InstanceID", 3))
        ...
        if (!((InstanceID) (obj)).zzvA().isEmpty())
        {
            ((InstanceID) (obj)).zzvA().zzvF();
            zzaf(true);
            return;
        }
...
_L3:
        ...
        if ("PING".equals(s))
        {
            try
            {
                GoogleCloudMessaging.getInstance(this).send(zzaAX, zzc.zzvE(), 0L, intent.getExtras());
                return;
            }
