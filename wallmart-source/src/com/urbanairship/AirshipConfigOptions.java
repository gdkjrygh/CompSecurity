...
_L2:
        if (inProduction)
        {
            ...
            {
                Logger.error((new StringBuilder()).append(productionLogLevel).append(" is not a valid log level. Falling back to ").append(6).append(" ERROR.").toString());
                productionLogLevel = 6;
            }
        } else
        if (!isLogLevelValid(developmentLogLevel))
        {
            Logger.error((new StringBuilder()).append(developmentLogLevel).append(" is not a valid log level. Falling back to ").append(3).append(" DEBUG.").toString());
            developmentLogLevel = 3;
        }
