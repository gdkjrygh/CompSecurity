...
//            IcsColorDrawable, AbsActionBarView, ActionBarView, ScrollingTabContainerView
...
public class ActionBarContainer extends NineFrameLayout
{
    ...
    public ActionBarContainer(Context context, AttributeSet attributeset)
    {
        ...
        {
            attributeset = Bitmap.createBitmap(1, 1, android.graphics.Bitmap.Config.ARGB_8888);
            Canvas canvas = new Canvas(attributeset);
            mStackedBackground.draw(canvas);
            int i = attributeset.getPixel(0, 0);
...
label0:
        {
            ...
            k1 = mTabContainer.getMeasuredHeight();
            if ((mActionBarView.getDisplayOptions() & 2) == 0)
            {
                ...
                mTabContainer.layout(i, 0, k, k1);
                if (i1 == 0)
                {
                    break label0;
                }
            }
            mTabContainer.layout(i, j1 - k1, k, j1);
        }
        if (!mIsSplit)
        {
            break MISSING_BLOCK_LABEL_361;
        }
        if (mSplitBackground == null) goto _L2; else goto _L1
_L1:
