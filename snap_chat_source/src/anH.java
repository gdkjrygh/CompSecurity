...
_L5:
        int l = k1 - k;
        byte byte2 = (byte)(abyte0[k1 - 1] & 0x7f);
        if (j == 0 || j == 1)
        {
            throw new anI((new StringBuilder("invalid padding byte '=' at byte offset ")).append(k).toString());
        }
        if (j == 3 && l > 2 || j == 4 && l > 1)
        {
            throw new anI((new StringBuilder("padding byte '=' falsely signals end of encoded value at offset ")).append(k).toString());
        }
        if (byte2 != 61 && byte2 != 10)
        {
            throw new anI("encoded value has invalid trailing byte");
        }
          goto _L2
_L6:
        int i1 = j + 1;
...
_L8:
        ...
        k = i1;
          goto _L7
_L4:
        throw new anI((new StringBuilder("Bad Base64 input character at ")).append(k).append(": ").append(abyte0[k]).append("(decimal)").toString());
_L2:
        k = i;
        if (j != 0)
        {
            if (j == 1)
            {
                throw new anI((new StringBuilder("single trailing character at offset ")).append(k1 - 1).toString());
            }
            abyte2[j] = 61;
