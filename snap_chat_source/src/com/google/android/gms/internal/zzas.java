...
public class zzas
{
    ...
    public static void zza(String as[], int i, int j, PriorityQueue priorityqueue)
    {
        ...
        {
            l = zza(zzaq.zzo(as[k - 1]), zzaq.zzo(as[(k + j) - 1]), l, l1, 0x1001fffL);
    ...
    {
        long l = ((long)zzaq.zzo(as[i]) + 0x7fffffffL) % 0x4000ffffL;
        for (int k = i + 1; k < i + j; k++)
        {
            l = ((l * 0x1001fffL) % 0x4000ffffL + ((long)zzaq.zzo(as[k]) + 0x7fffffffL) % 0x4000ffffL) % 0x4000ffffL;
        }
