...
public class StringUtil
{
    ...
    public static CharSequence replaceAll(CharSequence charsequence, String as[], CharSequence acharsequence[])
    {
        ...
        do
        {
            ...
            {
                String s = as[i];
                int j1 = TextUtils.indexOf(charsequence, s);
                if (j1 >= 0)
                {
                    flag = true;
                    charsequence.setSpan(s, j1, s.length() + j1, 33);
                }
            }
            ...
            if (flag)
            {
                ...
                while (j < as.length) 
                {
                    int k = charsequence.getSpanStart(as[j]);
                    int i1 = charsequence.getSpanEnd(as[j]);
                    if (k >= 0)
                    {
                        charsequence.replace(k, i1, acharsequence[j]);
                    }
                    j++;
                }
            } else
