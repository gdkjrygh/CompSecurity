...
final class <init>
{
    ...
    private int calculateMaxIndex()
    {
        ...
        while (i < k) 
        {
            ...
            j = Math.max(Math.max(Math.max(j, ((val) (obj)).min), ((val) (obj)).max), ((val) (obj)).size());
            i++;
        }
        if (j == -1)
        {
    ...
    private float calculateTotalWeight()
    {
        ...
        while (i < j) 
        {
            ...
            if (((View) (obj)).getVisibility() != 8)
            {
                ...
                f = ((tParams.rowSpec) (obj)).weight + f;
            }
            i++;
        }
        return f;
    ...
    private boolean computeHasWeights()
    {
        ...
        for (int i = 0; i < j; i++)
        {
            ...
            if (((tParams.rowSpec) (obj)).weight != 0.0F)
            {
                return true;
            }
        }
...
_L4:
    ...
    public final _cls1.sort[] getArcs()
    {
        if (arcs == null)
        {
            arcs = createArcs();
    ...
    public final dMap getGroupBounds()
    {
        if (groupBounds == null)
        {
...
label1:
    ...
    final trailingMargins[][] groupArcsByFirstVertex(trailingMargins atrailingmargins[])
    {
        boolean flag = false;
        int i1 = getCount() + 1;
        trailingMargins atrailingmargins1[][] = new getCount[i1][];
        int ai[] = new int[i1];
        int j1 = atrailingmargins.length;
        for (int i = 0; i < j1; i++)
        {
            int l1 = atrailingmargins[i].pan.min;
            ai[l1] = ai[l1] + 1;
        }

        for (int j = 0; j < i1; j++)
        {
            atrailingmargins1[j] = new val.min[ai[j]];
        }
        ...
        {
            trailingMargins trailingmargins = atrailingmargins[k];
            int l = trailingmargins.pan.min;
            trailingMargins atrailingmargins2[] = atrailingmargins1[l];
            int k1 = ai[l];
            ai[l] = k1 + 1;
            atrailingmargins2[k1] = trailingmargins;
        }
    ...
    public final void setCount(int i)
    {
        if (i != 0x80000000 && i < getMaxIndex())
        {
            ...
            GridLayout.access$100(stringbuilder.append(s).append("Count must be greater than or equal to the maximum of all grid indices (and spans) defined in the LayoutParams of each child").toString());
        }
        definedCount = i;
    }
