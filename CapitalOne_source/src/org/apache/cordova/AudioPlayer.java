...
label3:
    ...
    public void moveFile(String s)
    {
        ...
        if (Environment.getExternalStorageState().equals("mounted"))
        {
            file.renameTo(new File((new StringBuilder()).append(Environment.getExternalStorageDirectory().getAbsolutePath()).append(File.separator).append(s).toString()));
            return;
        } else
        {
            file.renameTo(new File((new StringBuilder()).append("/data/data/").append(handler.cordova.getActivity().getPackageName()).append("/cache/").append(s).toString()));
            return;
        }
    }
