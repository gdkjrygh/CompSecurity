...
public class FieldPacker
{
    ...
    static FieldPacker createFieldPack(Object aobj[])
    {
        boolean flag = false;
        int l = aobj.length;
        int i = 0;
        ...
        for (; i < l; i++)
        {
            k += getPackedSize(aobj[i]);
        }

        FieldPacker fieldpacker = new FieldPacker(k);
        k = aobj.length;
        for (int j = ((flag) ? 1 : 0); j < k; j++)
        {
            addToPack(fieldpacker, aobj[j]);
        }
    ...
    {
        while ((obj instanceof Boolean) || (obj instanceof Byte)) 
        {
            return 1;
        }
        if (obj instanceof Short)
        ...
        if (obj instanceof BaseObj)
        {
            return RenderScript.sPointerSize != 8 ? 4 : 32;
        } else
        {
            return 0;
        }
    ...
    public short subI16()
    {
        ...
        int j = mPos - 1;
        mPos = j;
        return (short)(i | (short)(abyte0[j] & 0xff));
    }
