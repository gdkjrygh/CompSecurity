...
_L1:
    ...
    private static TrustManager[] a(ah ah1)
    {
        try
        {
            TrustManagerFactory trustmanagerfactory = TrustManagerFactory.getInstance("X509");
            trustmanagerfactory.init(ah1.a);
            ah1 = trustmanagerfactory.getTrustManagers();
        }
        // Misplaced declaration of an exception variable
        catch (ah ah1)
        {
            throw new AssertionError(ah1);
        }
        // Misplaced declaration of an exception variable
        catch (ah ah1)
        {
...
_L1:
        cm.a().b().c("Crashlytics", (new StringBuilder("Certificate pins are stale, (")).append(System.currentTimeMillis() - c).append(" millis vs 15552000000").append(" millis) falling back to system trust.").toString());
_L4:
        e.add(ax509certificate[0]);
        return;
_L2:
        s = bu.a(ax509certificate, b);
        int l = s.length;
        int j = 0;
label0:
        do
