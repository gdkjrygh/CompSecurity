...
public class UiLifecycleHelper
{
    ...
    public UiLifecycleHelper(Activity activity1, Session.StatusCallback statuscallback)
    {
        ...
        } else
        {
            ...
            broadcastManager = j.a(activity1);
    ...
    private boolean handleFacebookDialogActivityResult(int i, int k, Intent intent, com.facebook.widget.FacebookDialog.Callback callback1)
    {
        com.facebook.widget.FacebookDialog.PendingCall pendingcall;
        if (pendingFacebookDialogCallId != null)
        {
    ...
    {
        onActivityResult(i, k, intent, null);
    }

    public void onActivityResult(int i, int k, Intent intent, com.facebook.widget.FacebookDialog.Callback callback1)
    {
        Session session = Session.getActiveSession();
        if (session != null)
        {
            session.onActivityResult(activity, i, k, intent);
        }
        if (LikeActionController.handleOnActivityResult(activity, i, k, intent))
        {
            return;
        } else
        {
            handleFacebookDialogActivityResult(i, k, intent, callback1);
            return;
        }
    }
