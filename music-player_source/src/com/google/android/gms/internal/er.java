...
public class er extends WebChromeClient
{
    ...
    private static boolean a(Context context, String s, String s1, String s2, JsResult jsresult, JsPromptResult jspromptresult, boolean flag)
    {
        ...
        if (flag)
        {
            ...
            s.addView(jsresult);
            s.addView(context);
            builder.setView(s).setPositiveButton(0x104000a, new fa(jspromptresult, context)).setNegativeButton(0x1040000, new ez(jspromptresult)).setOnCancelListener(new ex(jspromptresult)).create().show();
            return true;
        } else
        {
            builder.setMessage(s1).setPositiveButton(0x104000a, new ev(jsresult)).setNegativeButton(0x1040000, new eu(jsresult)).setOnCancelListener(new es(jsresult)).create().show();
            return true;
        }
    }
    ...
    protected final void a(View view, int i, android.webkit.WebChromeClient.CustomViewCallback customviewcallback)
    {
        ...
        if (bp1 == null)
        {
            ek.d("Could not get ad overlay when showing custom view.");
    ...
    public final void onCloseWindow(WebView webview)
    {
        ...
        {
            ek.d("Tried to close a WebView that wasn't an AdWebView.");
            return;
        }
        webview = ((em)webview).c();
        if (webview == null)
        {
            ek.d("Tried to close an AdWebView not associated with an overlay.");
            return;
        } else
        {
            webview.a();
    ...
    public final boolean onConsoleMessage(ConsoleMessage consolemessage)
    {
        ...
        fc.a[consolemessage.messageLevel().ordinal()];
...
_L6:
    ...
    public final void onHideCustomView()
    {
        ...
        if (bp1 == null)
        {
            ek.d("Could not get ad overlay when hiding custom view.");
            return;
        } else
        {
            bp1.c();
