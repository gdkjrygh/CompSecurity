...
label0:
    ...
    public static String encodeParameters(HttpParameter ahttpparameter[])
    {
        ...
        for (int i = 0; i < ahttpparameter.length; i++)
        {
            if (ahttpparameter[i].isFile())
            {
                throw new IllegalArgumentException("parameter [" + ahttpparameter[i].name + "]should be text");
            }
            if (i != 0)
            {
                stringbuffer.append("&");
            }
            stringbuffer.append(encode(ahttpparameter[i].name)).append("=").append(encode(ahttpparameter[i].value));
        }
    ...
    {
        return "PostParameter{name='" + name + '\'' + ", value='" + value + '\'' + ", file=" + file + ", fileBody=" + fileBody + '}';
    }
}
