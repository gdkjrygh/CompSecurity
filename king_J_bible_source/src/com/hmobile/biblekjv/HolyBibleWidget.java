...
public class HolyBibleWidget extends AppWidgetProvider
{
    ...
    private RemoteViews getRemoteViews(Context context)
    {
        if (remoteViews == null)
        {
            remoteViews = new RemoteViews(context.getPackageName(), 0x7f030037);
        }
        return remoteViews;
    }
    ...
    public static void updateWidget(RemoteViews remoteviews, Context context, AppWidgetManager appwidgetmanager)
    {
        ...
        if (todayverseinfo != null)
        {
            ...
            configIntent.putExtra("book_id", todayverseinfo.BookId);
            configIntent.putExtra("chap_id", todayverseinfo.ChapterNumber);
            configIntent.putExtra("verse_id", todayverseinfo.VerseNumber);
            configIntent.putExtra("isFromWidget", true);
            configIntent.setFlags(0x4000000);
            configIntent.setAction(ACTION_WIDGET_CONFIGURE);
            configPendingIntent = PendingIntent.getActivity(context, 1, configIntent, 0x8000000);
        } else
        {
        ...
        context = new ComponentName(context, com/hmobile/biblekjv/HolyBibleWidget);
        if (appwidgetmanager != null)
        {
