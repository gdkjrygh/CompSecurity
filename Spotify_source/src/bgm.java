// Decompiled by Jad v1.5.8e. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.geocities.com/kpdus/jad.html
// Decompiler options: braces fieldsfirst space lnc 

import android.os.IBinder;
import android.os.Parcel;
import com.google.android.gms.ads.internal.client.AdSizeParcel;

final class bgm
    implements bgk
{

    private IBinder a;

    bgm(IBinder ibinder)
    {
        a = ibinder;
    }

    public final IBinder a(byb byb1, AdSizeParcel adsizeparcel, String s, cex cex1, int i)
    {
        Object obj;
        Parcel parcel;
        Parcel parcel1;
        obj = null;
        parcel = Parcel.obtain();
        parcel1 = Parcel.obtain();
        parcel.writeInterfaceToken("com.google.android.gms.ads.internal.client.IAdManagerCreator");
        if (byb1 == null) goto _L2; else goto _L1
_L1:
        byb1 = byb1.asBinder();
_L5:
        parcel.writeStrongBinder(byb1);
        if (adsizeparcel == null) goto _L4; else goto _L3
_L3:
        parcel.writeInt(1);
        adsizeparcel.writeToParcel(parcel, 0);
_L6:
        parcel.writeString(s);
        byb1 = obj;
        if (cex1 == null)
        {
            break MISSING_BLOCK_LABEL_76;
        }
        byb1 = cex1.asBinder();
        parcel.writeStrongBinder(byb1);
        parcel.writeInt(i);
        a.transact(1, parcel, parcel1, 0);
        parcel1.readException();
        byb1 = parcel1.readStrongBinder();
        parcel1.recycle();
        parcel.recycle();
        return byb1;
_L2:
        byb1 = null;
          goto _L5
_L4:
        parcel.writeInt(0);
          goto _L6
        byb1;
        parcel1.recycle();
        parcel.recycle();
        throw byb1;
          goto _L5
    }

    public final IBinder a(byb byb1, AdSizeParcel adsizeparcel, String s, cex cex1, int i, int j)
    {
        Object obj;
        Parcel parcel;
        Parcel parcel1;
        obj = null;
        parcel = Parcel.obtain();
        parcel1 = Parcel.obtain();
        parcel.writeInterfaceToken("com.google.android.gms.ads.internal.client.IAdManagerCreator");
        if (byb1 == null) goto _L2; else goto _L1
_L1:
        byb1 = byb1.asBinder();
_L5:
        parcel.writeStrongBinder(byb1);
        if (adsizeparcel == null) goto _L4; else goto _L3
_L3:
        parcel.writeInt(1);
        adsizeparcel.writeToParcel(parcel, 0);
_L6:
        parcel.writeString(s);
        byb1 = obj;
        if (cex1 == null)
        {
            break MISSING_BLOCK_LABEL_76;
        }
        byb1 = cex1.asBinder();
        parcel.writeStrongBinder(byb1);
        parcel.writeInt(i);
        parcel.writeInt(j);
        a.transact(2, parcel, parcel1, 0);
        parcel1.readException();
        byb1 = parcel1.readStrongBinder();
        parcel1.recycle();
        parcel.recycle();
        return byb1;
_L2:
        byb1 = null;
          goto _L5
_L4:
        parcel.writeInt(0);
          goto _L6
        byb1;
        parcel1.recycle();
        parcel.recycle();
        throw byb1;
          goto _L5
    }

    public final IBinder asBinder()
    {
        return a;
    }
}
