    ...
    implements k
{
        ...
        implements Callable
    {
        ...
        public a(int ai[], int i, int j, int l, int i1, int j1, int k1)
        {
            a = ai;
            b = i;
            c = j;
    ...
    static void a(int ai[], int i, int j, int l, int i1, int j1, int k1)
    {
        b(ai, i, j, l, i1, j1, k1);
    }

    private static void b(int ai[], int i, int j, int l, int i1, int j1, int k1)
    {
        int k3 = i - 1;
        int l3 = j - 1;
        int i4 = l * 2 + 1;
        ...
        if (k1 == 1)
        {
            ...
            while (i1 < l3) 
            {
                ...
                {
                    ai1[j] = ai[j1];
                    l12 += (ai[j1] >>> 16 & 0xff) * (j + 1);
                    l11 += (ai[j1] >>> 8 & 0xff) * (j + 1);
                    l10 += (ai[j1] & 0xff) * (j + 1);
                    l4 += ai[j1] >>> 16 & 0xff;
                    l6 += ai[j1] >>> 8 & 0xff;
                    l5 += ai[j1] & 0xff;
                ...
                while (j <= l) 
                {
                    ...
                    ai1[j + l] = ai[k1];
                    l12 += (ai[k1] >>> 16 & 0xff) * ((l + 1) - j);
                    l11 += (ai[k1] >>> 8 & 0xff) * ((l + 1) - j);
                    l10 += (ai[k1] & 0xff) * ((l + 1) - j);
                    l9 += ai[k1] >>> 16 & 0xff;
                    l8 += ai[k1] >>> 8 & 0xff;
                    l7 += ai[k1] & 0xff;
                    j++;
    ...
    public Bitmap a(Bitmap bitmap, float f)
    {
        ...
        for (int i = 0; i < i1; i++)
        {
            bitmap.add(new a(ai, j, l, (int)f, i1, i, 1));
            arraylist.add(new a(ai, j, l, (int)f, i1, i, 2));
        }

        try
        {
            s.b.invokeAll(bitmap);
        }
