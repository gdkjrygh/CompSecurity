...
public class ShareToMessengerParams
{
    ...
    ShareToMessengerParams(ShareToMessengerParamsBuilder sharetomessengerparamsbuilder)
    {
        ...
        {
            throw new IllegalArgumentException((new StringBuilder("Unsupported mime-type: ")).append(mimeType).toString());
        }
        if (externalUri != null && !VALID_EXTERNAL_URI_SCHEMES.contains(externalUri.getScheme()))
        {
            throw new IllegalArgumentException((new StringBuilder("Unsupported external uri scheme: ")).append(externalUri.getScheme()).toString());
        } else
        {
            return;
        }
