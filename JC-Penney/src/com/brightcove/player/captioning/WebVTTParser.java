...
public class WebVTTParser
{
    ...
    private static final String a = com/brightcove/player/captioning/WebVTTParser.getSimpleName();
    private static final Pattern b = Pattern.compile("(\\d+:)?[0-5]\\d:[0-5]\\d\\.\\d{3}");
    ...
    public static WebVTTDocument parse(InputStream inputstream, String s)
    {
        ...
        {
            throw new IOException((new StringBuilder()).append("Expected WEBVTT. Got ").append(s).toString());
        }
        do
        {
            s = bufferedreader.readLine();
        ...
        do
        {
            ...
            if (matcher.find())
            {
                ...
                do
                {
                    ...
                    s = (new StringBuilder()).append(s).append(s2.trim()).append("\n").toString();
                } while (true);
